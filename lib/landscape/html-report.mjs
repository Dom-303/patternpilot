...
export function renderLandscapeHtml({ problem, landscape, runId, projectProfile = null, binding = null }) {
  ...
  // Lange Cluster-Labels wie "dedupe+python+query:record-linkage-library"
  // werden fuer die Nav-Leiste auf "Cluster N" verkuerzt — klickbar bleiben
  // die Links ueber die id-Anchors.
  const navItems = [
    ...
    ...clusters.map((_, i) => ({ href: `#cluster-${i + 1}`, label: `Cluster ${i + 1}` })),
