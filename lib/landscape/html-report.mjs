...
// ----------------------------------------------------------------------------
// Die Section-Reihenfolge, die Nav-Label-Konvention, der 21-Sections-Aufbau
// und die Cockpit-Night-Visuellen-Regeln sind mit Stand commit 22f6587
// (24.04.2026) final. Erlaubt sind weiterhin:
//   - Inhaltliche Qualitaetsverbesserungen je Section (bessere Daten, genauere
//     Empfehlungen, schaerfere Texte)
//   - Bugfixes fuer konkrete Rendering-Probleme
//   - Neue Builder-/Enrichment-Helper in lib/landscape/enrichment.mjs
// NICHT erlaubt ohne User-Freigabe:
//   - Umbau der Section-Reihenfolge
//   - Umbenennung oder Umstrukturierung bestehender Sections
//   - Austausch der Renderer-Helper (renderEmpfehlungenSection, renderAgentField etc.)
//   - Anpassung der Nav-Label-Konvention (1-2-Wort-Stil)
//   - Aenderungen an der max-2-Col-Grundregel
// Diese Regel gilt parallel fuer lib/html-renderer.mjs (Discovery/Review) und
// scripts/generate-styleguide.mjs (Styleguide). Kanonische Doku:
// docs/reference/TEMPLATE_LOCK.md
// ============================================================================
...
import {
  ...
  renderLandscapeEmpfehlungenSection,
...
export function renderLandscapeHtml({ problem, landscape, runId, projectProfile = null, binding = null }) {
  ...
  // Empfehlungen-Section — 3 Tabs auf REPO-Ebene, parallel zu Entscheidungen
  // (die auf Cluster-Ebene arbeitet). Gibt konkrete Action-Items pro Repo.
  const empfehlungenSection = renderLandscapeEmpfehlungenSection({
    clusters,
    topN: 5
  });

  // Cluster-Sections: eine pro Cluster. Redesign:
  //   - Relation-Badge EINMAL im Cluster-Head (statt pro Repo-Row) — alle Repos
  //     im selben Cluster haben dieselbe Relation, Wiederholung ist Rauschen.
  //   - Signature-Contrast als eigene Chip-Reihe zwischen Head und Repo-Liste —
  //     zeigt die unterscheidenden Signale des Clusters auf einen Blick.
  //   - Sub zeigt BEIDE vorhandenen Meta-Daten (Musterfamilie + Ebene) statt
  //     nur eines.
  //   - Repo-Rows clean: nur Name + klickbarer GitHub-Link, keine Badge-Dupes.
  const clusterSections = clusters.map((cluster, index) => {
    ...
    const signatureContrast = Array.isArray(cluster.signature_contrast) ? cluster.signature_contrast : [];
    ...
    const rows = members.length > 0
      ? members.map((memberId) => {
          ...
          let url = "";
          let label = raw;
          const httpMatch = raw.match(/^https?:\/\/github\.com\/([^/\s]+)\/([^/\s#?]+)/i);
          if (httpMatch) {
            url = `https://github.com/${httpMatch[1]}/${httpMatch[2].replace(/\.git$/, "")}`;
            label = `${httpMatch[1]}/${httpMatch[2].replace(/\.git$/, "")}`;
    ...
    // Sub: beide Meta-Infos zeigen wenn vorhanden, mit Trenner.
    const subParts = [];
    if (cluster.pattern_family) subParts.push(`Musterfamilie · ${cluster.pattern_family}`);
    if (cluster.main_layer) subParts.push(`Ebene · ${cluster.main_layer}`);
    const sub = subParts.join("  |  ");

    // Signature-Contrast als Chip-Reihe (einmal pro Cluster, nicht pro Repo).
    const contrastStrip = signatureContrast.length > 0
      ? `<div class="landscape-cluster-contrast"><span class="landscape-cluster-contrast-label">Unterscheidende Signale</span>${signatureContrast.slice(0, 6).map((s) => `<span class="landscape-cluster-contrast-chip">${escapeHtml(s)}</span>`).join("")}</div>`
      : "";
  ...
  <div class="section-head">
    ...
    <div class="head-actions">
      <span class="badge ${relBadge.tone}">${escapeHtml(relBadge.label)}</span>
      <span class="count-chip">${members.length} Repos</span>
    </div>
  </div>
  ...
  const achsenCountChip = validAxisDimensions.length > 0
    ? `${validAxisDimensions.length} Achsen`
    : "keine Achsen";
  const achsenSection = `<section class="section-preview accent-purple" id="achsen" data-nav-section${achsenEmpty}>
  <div class="section-head">
    <div class="title">
      <h2><span class="marker"></span>Achsen-View</h2>
      <div class="sub">Lage des Loesungsraums auf den Schluessel-Dimensionen</div>
    </div>
    <div class="head-actions"><span class="count-chip">${achsenCountChip}</span></div>
  </div>
  ${renderInlineSectionDescription("achsen")}
  <div class="section-body">
    ${validAxisDimensions.length > 0
      ? validAxisDimensions.map((dim) => renderAxisRow(dim)).join("\n")
      : `<p class="empty">Keine Achsen-Dimensionen verfuegbar. Die Achsen-View entsteht aus <code>suspected_approach_axes</code> in der problem.md — ergaenze dort 2-4 Achsen (z.B. "matching_strategy: rule-based &harr; ML-embedding") und fahre problem:explore erneut.</p>`}
  </div>
</section>`;
  ...
  // Problem-Details: beschreibt das Problem ausfuehrlich fuer den Leser,
  // statt nur Slug/Titel anzuzeigen. Parity zum Discovery-Report, wo der
  // Projekt-Kontext sichtbar ist — hier der analoge Problem-Kontext.
  //
  // Grundregel max 2 Col: info-grid statt meta-grid, damit die 5 Felder als
  // echte Karten in 2-Spalten-Raster sitzen (und bei schmalem Viewport auf
  // 1 Spalte umbrechen), statt als schmaler Key-Value-Stack.
  const fields = problem.fields ?? {};
  const problemDetailCards = [];
  if (fields.description) {
    problemDetailCards.push({ title: "Beschreibung", copy: fields.description });
  }
  if (fields.current_approach) {
    problemDetailCards.push({ title: "Aktueller Ansatz", copy: fields.current_approach });
  }
  if (fields.success_criteria) {
    const items = Array.isArray(fields.success_criteria) ? fields.success_criteria : [fields.success_criteria];
    problemDetailCards.push({ title: "Erfolgskriterien", items });
  }
  if (Array.isArray(fields.constraints) && fields.constraints.length > 0) {
    problemDetailCards.push({ title: "Constraints", items: fields.constraints });
  }
  if (Array.isArray(fields.non_goals) && fields.non_goals.length > 0) {
    problemDetailCards.push({ title: "Nicht-Ziele", items: fields.non_goals });
  }
  const problemDetailsEmpty = problemDetailCards.length === 0 ? ' data-section-empty="true"' : "";
  const problemDetailsSection = `<section class="section-preview accent-cyan" id="problem-details" data-nav-section${problemDetailsEmpty}>
  <div class="section-head">
    <div class="title">
      <h2><span class="marker"></span>Problem-Details</h2>
      <div class="sub">Beschreibung, Ansatz, Kriterien, Constraints</div>
    </div>
    <div class="head-actions"><span class="count-chip">${problemDetailCards.length} Felder</span></div>
  </div>
  ${renderInlineSectionDescription("problem-details")}
  <div class="section-body">
    ${problemDetailCards.length > 0
      ? renderInfoGrid(problemDetailCards)
      : `<p class="empty">Keine Problem-Details im problem.md gepflegt. Ergaenze description, current_approach und success_criteria, um diesen Abschnitt zu aktivieren.</p>`}
  </div>
</section>`;

  // Staerkste Risikosignale (Landscape-Version): Cluster-level Warnungen.
  // Parity zu Discovery "highest-risk-signals". Quellen fuer Risikosignale:
  //   - has_constraint_violation (aus dem Clustering-Pipeline)
  //   - Singletons (Cluster mit nur 1 Mitglied — schwache Evidenz)
  //   - "unknown"-Cluster (ohne klare pattern_family / main_layer)
  const riskSignals = [];
  for (const c of clusters) {
    const label = c.label ?? "unlabeled";
    if (c.has_constraint_violation) {
      riskSignals.push(`${label}: Cluster enthaelt Mitglieder mit Constraint-Warnungen — Lizenz/Host pruefen`);
    }
    const memberCount = c.member_ids?.length ?? 0;
    if (memberCount === 1) {
      riskSignals.push(`${label}: nur 1 Mitglied — Musterfamilie auf Einzel-Repo-Basis, schwache Evidenz`);
    }
    const isUnknown = (!c.pattern_family || c.pattern_family === "unknown") && (!c.main_layer || c.main_layer === "unknown");
    if (isUnknown && memberCount >= 2) {
      riskSignals.push(`${label}: keine klare Musterfamilie/Ebene zugeordnet — Cluster-Bedeutung unklar`);
    }
  }
  if (landscape.landscape_signal === "single_cluster_collapse") {
    riskSignals.push("Landscape-Signal: single_cluster_collapse — nur 1 Haupt-Cluster gebildet, Diversitaet fehlt. Siehe Tech-Status fuer konkrete Hebel.");
  }
  const riskSignalsEmpty = riskSignals.length === 0 ? ' data-section-empty="true"' : "";
  const riskSignalsSection = `<section class="section-preview accent-orange" id="highest-risk-signals" data-nav-section${riskSignalsEmpty}>
  <div class="section-head">
    <div class="title">
      <h2><span class="marker"></span>Staerkste Risikosignale</h2>
      <div class="sub">Cluster-Warnungen und Landscape-Schwaechen</div>
    </div>
    <div class="head-actions"><span class="count-chip">${riskSignals.length > 0 ? `${riskSignals.length} Signale` : "keine Risiken"}</span></div>
  </div>
  ${renderInlineSectionDescription("highest-risk-signals")}
  <div class="section-body">
    ${renderHtmlList(riskSignals, "Keine Risikosignale fuer diese Landscape — robuste Cluster-Basis.")}
  </div>
</section>`;

  // Zielrepo-Kontext: zeigt, welche Projekt-Dateien Pattern Pilot wirklich
  // gelesen hat (genau wie im Discovery-Report). Nur wenn projectProfile
  // + binding durch problem-explore.mjs geladen wurden.
  const hasProjectContext = Boolean(projectProfile && binding);
  const targetRepoContextSection = `<section class="section-preview accent-orange" id="target-repo-context" data-nav-section${hasProjectContext ? "" : ' data-section-empty="true"'}>
  <div class="section-head">
    <div class="title">
      <h2><span class="marker"></span>Zielrepo-Kontext</h2>
      <div class="sub">${escapeHtml(projectKey)} · welche Dateien wurden gelesen</div>
    </div>
    <div class="head-actions"><span class="count-chip">${hasProjectContext ? "Kontext" : "kein Binding"}</span></div>
  </div>
  ${renderInlineSectionDescription("target-repo-context")}
  <div class="section-body">
    ${hasProjectContext
      ? renderProjectContextSources(projectProfile, binding)
      : `<p class="empty">Kein Projekt-Binding geladen — Problem laeuft im Standalone-Modus. Bindings leben unter <code>bindings/&lt;project&gt;/PROJECT_BINDING.json</code>.</p>`}
  </div>
</section>`;

  // Coverage: Verteilung der Cluster nach main_layer und Relation — macht
  // sichtbar, welche Loesungs-Schichten die Landscape abdeckt und wie die
  // Beziehung zum aktuellen Ansatz verteilt ist. Aggregiert aus den Cluster-
  // Daten. Labels sind Landscape-spezifisch (nicht Discovery-Faehigkeiten).
  const coverageMainLayers = new Map();
  const coverageRelations = new Map();
  const relationDisplay = {
    divergent: "Divergent",
    adjacent: "Benachbart",
    near_current_approach: "Nah am aktuellen Ansatz"
  };
  for (const c of clusters) {
    if (c.main_layer) coverageMainLayers.set(c.main_layer, (coverageMainLayers.get(c.main_layer) ?? 0) + (c.member_ids?.length ?? 0));
    if (c.relation) {
      const display = relationDisplay[c.relation] ?? c.relation;
      coverageRelations.set(display, (coverageRelations.get(display) ?? 0) + 1);
    }
  }
  const coverageData = {
    mainLayers: [...coverageMainLayers.entries()].map(([value, count]) => ({ value, count })).sort((a, b) => b.count - a.count),
    capabilities: [...coverageRelations.entries()].map(([value, count]) => ({ value, count })).sort((a, b) => b.count - a.count),
    gapAreas: []
  };
  const coverageEmpty = coverageData.mainLayers.length === 0 && coverageData.capabilities.length === 0;
  const coverageSection = `<section class="section-preview accent-purple" id="coverage" data-nav-section${coverageEmpty ? ' data-section-empty="true"' : ""}>
  <div class="section-head">
    <div class="title">
      <h2><span class="marker"></span>Coverage</h2>
      <div class="sub">Loesungs-Schichten und Beziehungs-Verteilung</div>
    </div>
    <div class="head-actions"><span class="count-chip">${coverageData.mainLayers.length} Schichten &middot; ${coverageData.capabilities.length} Beziehungen</span></div>
  </div>
  ${renderInlineSectionDescription("coverage")}
  <div class="section-body">
    ${coverageEmpty
      ? `<p class="empty">Keine Coverage-Daten — Landscape enthaelt keine Cluster mit main_layer- oder Relation-Zuordnung.</p>`
      : renderCoverageCards(coverageData, {
          titles: {
            mainLayers: "Loesungs-Schichten (Repos pro main_layer)",
            capabilities: "Beziehung zum aktuellen Ansatz (Cluster je Kategorie)"
          }
        })}
  </div>
</section>`;
  ...
  // Problem-Slug generiert wurden, plus Reasons + Treffer-Zahl pro Query.
  // Sortiert (in enrichment.mjs) nach hitCount descending, damit die
  // produktivsten Linsen zuerst erscheinen.
  ...
  const totalQueryHits = queryPlans.reduce((sum, q) => sum + (q.hitCount ?? 0), 0);
  const productiveQueries = queryPlans.filter((q) => (q.hitCount ?? 0) >= 1).length;
  const queryPlansCountChip = queryPlans.length > 0
    ? `${queryPlans.length} Linsen · ${productiveQueries} mit Treffern · ${totalQueryHits} Repos`
    : "leer";

  // Custom Card-Rendering fuer Problem-Linsen: Query-Text als <code>-Span,
  // Treffer-Badge oben rechts (adopt/adapt/observe-Tones je nach Productivity),
  // Reasons als Liste. Umgeht renderInfoCard, weil das keine Badge-Slots hat.
  const hitBadgeTone = (hits) => hits >= 3 ? "adopt" : hits >= 1 ? "adapt" : "observe";
  const queryCards = queryPlans.map((q) => {
    const hits = q.hitCount ?? 0;
    const hitTone = hitBadgeTone(hits);
    const reasonsList = Array.isArray(q.reasons) && q.reasons.length > 0
      ? `<ul class="info-card-list">${q.reasons.map((r) => `<li>${escapeHtml(r)}</li>`).join("")}</ul>`
      : `<p class="info-card-empty">Keine Gruende erfasst.</p>`;
    return `<div class="info-card">
  <div class="info-card-title" style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">
    <span>${escapeHtml(q.label ?? "Query")}</span>
    <span class="badge ${hitTone}">${hits} Treffer</span>
  </div>
  <pre class="agent-pre" style="margin:8px 0 12px;padding:10px 12px;font-size:11.5px"><code>${escapeHtml(q.query ?? "")}</code></pre>
  ${reasonsList}
</div>`;
  }).join("\n");
  ...
  <div class="section-head">
    <div class="title">
      ...
      <div class="sub">Query-Phrasen + Trefferzahl pro Linse</div>
    </div>
    <div class="head-actions"><span class="count-chip">${queryPlansCountChip}</span></div>
  </div>
  ${renderInlineSectionDescription("landscape-lenses")}
  <div class="section-body">
    ${queryPlans.length > 0
      ? `<div class="info-grid">${queryCards}</div>`
  ...
  // Nav-Labels bewusst knapp (1-2 Woerter), konsistent zum Discovery-Report.
  // Cluster-Nav-Labels: pattern_family (verkuerzt) statt generischem
  // "Cluster N" — damit der Nutzer in der Nav sofort sieht, worum es im
  // jeweiligen Cluster geht. Fallback auf Signature-Contrast-Erstwort
  // oder "Cluster N" wenn nichts Sprechendes da ist.
  const shortClusterNavLabel = (cluster, index) => {
    const fallback = `Cluster ${index + 1}`;
    // pattern_family ist typisch "python record linkage" oder "fuzzy matching"
    // — ideal fuer die Nav, nach 2-3 Worten gekappt.
    if (cluster.pattern_family && cluster.pattern_family !== "unknown") {
      const words = String(cluster.pattern_family).split(/\s+/).filter(Boolean).slice(0, 3);
      const joined = words.join(" ");
      if (joined.length > 0 && joined.length <= 22) return joined;
      if (joined.length > 22) return joined.slice(0, 21).trim() + "…";
    }
    // Sonst: erstes signifikantes Signature-Contrast-Keyword
    const contrast = Array.isArray(cluster.signature_contrast) ? cluster.signature_contrast : [];
    for (const kw of contrast) {
      if (typeof kw === "string" && !kw.startsWith("query:") && kw.length >= 3 && kw.length <= 22) {
        return kw.replace(/-/g, " ");
      }
    }
    return fallback;
  ...
  const navItems = [
    ...
    { href: "#landscape-filter", label: "Filter" },
    { href: "#uebersicht", label: "Uebersicht" },
    { href: "#problem-details", label: "Details" },
    { href: "#entscheidungen", label: "Entscheidungen" },
    { href: "#empfehlungen", label: "Empfehlungen" },
    { href: "#landscape-lenses", label: "Linsen" },
    ...clusters.map((c, i) => ({ href: `#cluster-${i + 1}`, label: shortClusterNavLabel(c, i) })),
    { href: "#coverage", label: "Coverage" },
    { href: "#achsen", label: "Achsen" },
    { href: "#highest-risk-signals", label: "Risiken" },
    { href: "#target-repo-context", label: "Kontext" },
    { href: "#run-health", label: "Gesundheit" },
    { href: "#tech-status", label: "Status" },
    { href: "#agent-view", label: "Agenten" },
    { href: "#lauf", label: "Lauf" },
