...
export function renderLandscapeHtml({ problem, landscape, runId, projectProfile = null, binding = null }) {
  ...
  const problemDetailItems = [
    fields.description ? { key: "Beschreibung", value: fields.description } : null,
    fields.current_approach ? { key: "Aktueller Ansatz", value: fields.current_approach } : null,
    fields.success_criteria ? { key: "Erfolgskriterien", value: Array.isArray(fields.success_criteria) ? fields.success_criteria.join(" · ") : fields.success_criteria } : null,
    Array.isArray(fields.constraints) && fields.constraints.length > 0 ? { key: "Constraints", value: fields.constraints.join(" · ") } : null,
    Array.isArray(fields.non_goals) && fields.non_goals.length > 0 ? { key: "Nicht-Ziele", value: fields.non_goals.join(" · ") } : null
  ].filter(Boolean);
  const problemDetailsEmpty = problemDetailItems.length === 0 ? ' data-section-empty="true"' : "";
  ...
  <div class="section-head">
    ...
    <div class="head-actions"><span class="count-chip">${problemDetailItems.length} Felder</span></div>
  </div>
  ${renderInlineSectionDescription("problem-details")}
  <div class="section-body">
    ${problemDetailItems.length > 0
      ? renderMetaGrid(problemDetailItems)
