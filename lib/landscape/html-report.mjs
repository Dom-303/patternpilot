...
export function renderLandscapeHtml({ problem, landscape, runId, projectProfile = null, binding = null }) {
  ...
  const clusterSections = clusters.map((cluster, index) => {
    ...
    const rows = members.length > 0
      ? members.map((memberId) => {
          // member_ids sind heute Full-URLs ("https://github.com/owner/repo"),
          // koennen historisch auch als "owner/repo"-Slug vorkommen. Beide Formen
          // akzeptieren und zu einer klickbaren URL + lesbarem Owner/Name-Label
          // aufloesen, damit die Repo-Links immer sauber klickbar sind.
  ...
  // sichtbar, welche Loesungs-Schichten die Landscape abdeckt und wo Luecken
  // liegen. Aggregiert aus den Cluster-Daten.
  ...
  for (const c of clusters) {
    ...
    if (c.relation) coverageRelations.set(c.relation, (coverageRelations.get(c.relation) ?? 0) + 1);
  }
  const coverageData = {
  ...
  <div class="section-head">
    <div class="title">
      ...
      <div class="sub">Verteilung nach Ebene und Relation</div>
    </div>
    <div class="head-actions"><span class="count-chip">${coverageData.mainLayers.length} Layer &middot; ${coverageData.capabilities.length} Relationen</span></div>
  </div>
