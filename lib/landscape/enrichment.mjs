 ...
 * Suchfehlern.
 */
export function buildLandscapeTechStatus({ queries, candidateCount, clusterCount, outlierCount, passError, landscapeSignal }) {
  ...
  if ((candidateCount ?? 0) > 0 && (clusterCount ?? 0) <= 1) {
    ...
    // Nicht automatisch ein Fehler — kann auch heissen, dass der Loesungs-
    // raum wirklich nur eine dominante Familie hat. Konkreter Leitfaden:
    missingCandidates.push("Nur 1 Cluster gebildet — die Loesungslandschaft ist thematisch homogen.");
    missingCandidates.push("Naechster Hebel: Query-Seeds orthogonal erweitern (z.B. andere Ansatz-Tags, andere Sprache-Oekosysteme, andere Problem-Formulierung).");
    missingCandidates.push("Alternativ: Discovery-Depth erhoehen (--depth deep) oder Signature-Approach gegenchecken.");
  }
