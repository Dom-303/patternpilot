 ...
 * Jede Query bekommt eine menschliche Begruendung und — wenn repos mit
 * discoveryProvenance vorliegen — die Anzahl der Repos, die diese Query
 * in den Pool gebracht hat. Das hitCount-Feld macht sichtbar, welche Linsen
 * produktiv waren und welche leer gelaufen sind.
 */
export function buildLandscapeQueryPlans(queries, problem, repos = []) {
  ...
  // Pro Query zaehlen, wie viele selektierte Repos sie als Primary-Linse
  // haben. Repos koennen von mehreren Queries gefunden worden sein, aber
  // hier zaehlt nur die erste Query (discoveryProvenance[0]), damit jede
  // Zuordnung disjunkt ist.
  const hitsByQuery = new Map();
  for (const r of Array.isArray(repos) ? repos : []) {
    const prov = Array.isArray(r?.discoveryProvenance) ? r.discoveryProvenance : [];
    if (prov.length === 0) continue;
    const primary = prov[0];
    hitsByQuery.set(primary, (hitsByQuery.get(primary) ?? 0) + 1);
  }
 ...
 * Suchfehlern.
 */
export function buildLandscapeTechStatus({
  queries,
  candidateCount,
  clusterCount,
  outlierCount,
  ...
  if ((candidateCount ?? 0) > 0 && (clusterCount ?? 0) <= 1) {
    // single_cluster_collapse: die Landschaft ist thematisch homogen.
    // Konkrete, kopierbare Naechste-Schritte-Vorschlaege — inklusive
    // Live-CLI-Commands, damit der User direkt weiter machen kann.
    missingCandidates.push("Nur 1 Cluster gebildet — die Loesungslandschaft ist thematisch homogen.");
    const slug = problemSlug ?? "<slug>";
    const project = projectKey ?? "<project>";
    missingCandidates.push(
      `Naechster Hebel A (mehr Kandidaten pro Linse): npm run problem:explore -- ${slug} --project ${project} --per-page 50`
    );
    missingCandidates.push(
      `Naechster Hebel B (breitere Query-Familie): npm run problem:explore -- ${slug} --project ${project} --depth deep`
    );
    missingCandidates.push(
      `Naechster Hebel C (orthogonale Seeds): in projects/${project}/problems/${slug}/problem.md die hints.search_terms um andere Ansatz-Tags / Sprach-Oekosysteme / Problem-Formulierungen ergaenzen, dann erneut ausfuehren.`
    );
  }
