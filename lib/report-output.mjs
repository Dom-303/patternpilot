...
function renderBrowserLinkMarkdown(state) {
  const lines = [
    ...
    "",
    "> Pfad markieren, Strg+C, in die Adressleiste deines Browsers einfuegen.",
    ""
  ];
  if (state.updated_at) {
  ...
  for (const [sectionKey, entries] of sectionEntries) {
    ...
    for (const [entryKey, entry] of entryList) {
      ...
      // Original-Format, genau eine Pfad-Zeile pro Eintrag, plain text,
      // keine Prefixe, keine Backticks. Das ist die Form, die vom ersten
      // Tag an sauber in Windows-Browser/Explorer eingefuegt werden konnte.
      lines.push(`- **${label}**${updated}`, `  ${entry.href}`, "");
    }
  }
  return `${lines.join("\n")}\n`;
}
...
// Statische Eintraege, die bei jedem pushBrowserLink mitgefuehrt werden —
// unabhaengig von konkreten Laeufen. Aktuell: das Design-System / der
// Report-UI-Styleguide, der als committed Doku-Artefakt jederzeit im Repo
// liegt und vom Nutzer direkt im Browser geoeffnet werden soll.
function staticEntriesForRootDir(rootDir) {
  if (!rootDir) return [];
  const styleguideAbs = path.resolve(rootDir, "docs/reference/REPORT_UI_TOKENS.html");
  const discoveryDemoAbs = path.resolve(rootDir, "runs/_ui-test/full-sections-smoke.html");
  const landscapeDemoAbs = path.resolve(rootDir, "runs/_ui-test/landscape-demo-smoke.html");
  return [
    {
      section: "docs",
      key: "ui-styleguide",
      label: "UI-Styleguide (Cockpit Night Design System)",
      href: buildBrowserLinkTarget(styleguideAbs)
    },
    {
      section: "docs",
      key: "discovery-demo",
      label: "Discovery-Demo (alle Sections voll befuellt)",
      href: buildBrowserLinkTarget(discoveryDemoAbs)
    },
    {
      section: "docs",
      key: "landscape-demo",
      label: "Landscape-Demo (alle Sections voll befuellt)",
      href: buildBrowserLinkTarget(landscapeDemoAbs)
    }
  ];
}

function mergeEntryIntoState(state, entry, now) {
  state.sections[entry.section] = state.sections[entry.section] ?? {};
  const existing = state.sections[entry.section][entry.key];
  // Statische Eintraege (docs) nicht staendig re-touchen — nur einsetzen,
  // falls sie fehlen oder ihr Pfad sich geaendert hat.
  if (existing && existing.href === entry.href) return false;
...
export async function pushBrowserLink(browserLinkPath, entry, options = {}) {
  ...
  // Statische Doku-Links einmischen, wenn der Caller einen rootDir mitgibt.
  // Dadurch erscheint der Styleguide bei jedem Lauf automatisch im
  // browser-link-File, ohne dass ein Command ihn aktiv pushen muss.
  if (options.rootDir) {
    for (const staticEntry of staticEntriesForRootDir(options.rootDir)) {
      mergeEntryIntoState(state, staticEntry, now);
    }
  }
