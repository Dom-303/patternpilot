...
// Baut aus einem Browser-Link-Href eine klickbare URL fuer die Markdown-
// Link-Syntax. Ziel: auf Windows UND Linux/macOS soll ein Klick im
// Markdown-Viewer (VS Code, GitHub, GitLab) den Report direkt im Browser
// oeffnen, ohne dass der Nutzer copy-pasten muss.
//   - WSL-UNC  `\\wsl.localhost\<distro>\<path>`  ->  `file://wsl.localhost/<distro>/<path>`
//   - Windows-Drive `C:\...`                       ->  `file:///C:/...`
//   - file://-URI (schon korrekt)                  ->  passthrough
//   - POSIX-Pfad `/home/...`                       ->  `file:///home/...`
//   - alles andere (relativ etc.)                  ->  as-is
function hrefToClickableUrl(href) {
  if (typeof href !== "string" || !href) return href;
  if (href.startsWith("file://")) return href;
  if (href.startsWith("\\\\wsl.localhost\\") || href.startsWith("\\\\wsl$\\")) {
    const stripped = href.replace(/^\\\\/, "");
    return `file://${stripped.replace(/\\/g, "/")}`;
  }
  if (href.startsWith("\\\\")) {
    return `file:${href.replace(/\\/g, "/")}`;
  }
  const driveMatch = href.match(/^([a-zA-Z]):[\\/](.*)$/);
  if (driveMatch) {
    return `file:///${driveMatch[1]}:/${driveMatch[2].replace(/\\/g, "/")}`;
  }
  if (href.startsWith("/")) {
    return `file://${href.replace(/ /g, "%20")}`;
  }
  return href;
}

function renderBrowserLinkMarkdown(state) {
  const lines = ["# Pattern Pilot — Browser Links", ""];
  if (state.updated_at) {
  ...
  for (const [sectionKey, entries] of sectionEntries) {
    ...
    for (const [entryKey, entry] of entryList) {
      ...
      // Zwei Zeilen pro Eintrag:
      //   1. Klickbarer Markdown-Link, der direkt im Browser oeffnet
      //   2. Raw-Pfad in Backticks — weiter copy-pasten fuer Windows-Explorer
      const clickable = hrefToClickableUrl(entry.href);
      lines.push(
        `- **${label}**${updated}`,
        `  [Open report](${clickable})`,
        `  \`${entry.href}\``,
        ""
      );
    }
