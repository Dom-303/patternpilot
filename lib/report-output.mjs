...
function renderBrowserLinkMarkdown(state) {
  const lines = [
    ...
    "",
    "> **So oeffnest du einen Report:**",
    "> - **Windows Explorer / Browser:** kopiere den `\\\\wsl.localhost\\...`-Pfad, paste in Explorer-Adresszeile oder Browser-URL-Leiste.",
    "> - **WSL / Linux Shell:** kopiere den POSIX-Pfad und fuehre `wslview <pfad>` (oeffnet im Windows-Browser) oder `xdg-open <pfad>` (Linux-Browser) aus.",
    "> - _Hinweis:_ VS Code blockiert `file://`-Links in der Markdown-Preview — Click funktioniert dort nicht, Copy-Paste schon.",
    ""
  ];
  if (state.updated_at) {
  ...
  for (const [sectionKey, entries] of sectionEntries) {
    ...
    for (const [entryKey, entry] of entryList) {
      ...
      // Zwei Copy-Paste-Pfade pro Eintrag — bewusst KEINE klickbaren
      // Markdown-Links, weil VS Code `file://`-URIs in der Preview
      // blockiert. Windows-User greifen den UNC-Pfad, WSL-User den
      // POSIX-Pfad.
      const posix = hrefToPosixPath(entry.href);
      const lineBlock = [`- **${label}**${updated}`, `  - Windows: \`${entry.href}\``];
      if (posix) lineBlock.push(`  - WSL:     \`${posix}\``);
      lineBlock.push("");
