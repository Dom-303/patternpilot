...
function renderSectionExplainButton(sectionId, fallbackTitle) {
  // Behalten fuer Backwards-Compatibility, NICHT mehr genutzt von den
  // Wrapper-Funktionen. Section-Erklaerung laeuft jetzt ueber den
  // Description-Collapse (renderInlineSectionDescription unten).
...
// Inline-Section-Description: collapsible Dropdown unter dem section-head.
// Ersetzt den frueheren Info-Button + Modal-Mechanismus pro Section.
export function renderInlineSectionDescription(sectionId, { extraBody = "" } = {}) {
  const info = getSectionInfo(sectionId);
  if (!info && !extraBody) return "";
  const body = info
    ? `<p>${escapeHtml(info.body)}</p>${extraBody}`
    : extraBody;
...
// Semantic accent map: KPI label keywords decide color, not index order.
// Reihenfolge wichtig: erster match gewinnt. Lower-case-Keyword -> accent.
const STAT_ACCENT_KEYWORDS = [
  ["risik", "orange"],
  ["warn", "orange"],
  ["fehler", "orange"],
  ["uebernahm", "green"],
...
const STAT_ACCENT_FALLBACK_ROTATION = ["magenta", "purple", "orange", "green", "cyan", "mixed"];

function resolveStatAccent(label, index) {
  const key = String(label ?? "").toLowerCase();
  for (const [needle, accent] of STAT_ACCENT_KEYWORDS) {
    if (key.includes(needle)) return accent;
  }
  return STAT_ACCENT_FALLBACK_ROTATION[index % STAT_ACCENT_FALLBACK_ROTATION.length];
}
...
export function renderHtmlStatCards(stats) {
  return stats
    .map((stat, index) => {
      ...
      const accent = resolveStatAccent(stat.label, index);
...
// Heuristik: wenn der Section-Body nur aus einem <p class="empty">-Paragraphen
// besteht, gilt die Section als leer. Das Flag triggert den Filter-Toolbar-
// Switch "Auch leere Felder anzeigen" (default: blenden sie aus).
function bodyLooksEmpty(body) {
  if (!body) return true;
  const trimmed = String(body).trim();
  if (!trimmed) return true;
  return /^<p class="empty">[^<]*<\/p>$/.test(trimmed);
}
...
export function renderHtmlSection(title, body, tone = "default", sectionId = "") {
  ...
  const emptyAttr = bodyLooksEmpty(body) ? ' data-section-empty="true"' : "";
  const accentClass = sectionPreviewAccent(sectionId, tone);
  const description = renderInlineSectionDescription(sectionId);
  return `<section class="section-preview ${accentClass}"${idAttr}${emptyAttr}>
...
export function buildBadgeExplainAttrs(tone) {
  ...
  if (!explain) return "";
  return ` aria-haspopup="dialog" data-explain-title="${escapeHtml(explain.title)}" data-explain-body="${escapeHtml(explain.body)}"`;
}
...
export function renderDecisionSummary({ candidates, reportType, runRoot, discovery, review }) {
  ...
  if (!hasDecisionRunFields(root)) {
    ...
    return `<section class="section-preview accent-orange" id="decision-summary" data-nav-section data-section-empty="true">
  <div class="section-head">
    <div class="title">
  ...
  ${renderInlineSectionDescription("decision-summary")}
...
export function renderRecommendedActions({ candidates, reportType, runRoot, discovery, review, bodyOnly = false } = {}) {
  const root = getRunRoot({ reportType, runRoot, discovery, review });
  const safeCandidates = Array.isArray(candidates) ? candidates : [];

  // Immer Section emittieren, damit Sidenav und Content konsistent bleiben.
  // Empty-State: entweder keine Kandidaten ODER Engine-Daten fehlen.
  if (!hasDecisionRunFields(root) || safeCandidates.length === 0) {
    const reason = !hasDecisionRunFields(root)
      ? "Engine-Daten unvollstaendig — dieser Lauf wurde vor der Engine-Upgrade-Integration erzeugt. Lauf erneut ausfuehren, um Handlungsempfehlungen zu bekommen."
      : "Keine Kandidaten in diesem Lauf vorhanden. Sobald Discovery Treffer liefert oder die Watchlist gefuellt ist, erscheinen hier gruppierte Handlungsempfehlungen.";
    if (bodyOnly) {
      return `<p class="empty">${escapeHtml(reason)}</p>`;
    }
    return `<section class="section-preview accent-green" id="recommended-actions" data-nav-section data-section-empty="true">
  <div class="section-head">
    <div class="title">
      <h2><span class="marker"></span>Empfohlene Aktionen</h2>
      <div class="sub">Keine Handlungsempfehlung verfuegbar</div>
    </div>
  </div>
  ${renderInlineSectionDescription("recommended-actions")}
  ...
  if (configs.length === 0) {
    const emptyBody = `<p class="empty">Keine Kandidaten in den Dispositions-Gruppen — pruefe die Discovery-Daten.</p>`;
    if (bodyOnly) return emptyBody;
    return `<section class="section-preview accent-green" id="recommended-actions" data-nav-section data-section-empty="true">
  <div class="section-head">
    <div class="title">
      <h2><span class="marker"></span>Empfohlene Aktionen</h2>
      <div class="sub">Keine Kandidaten in Dispositions-Gruppen</div>
    </div>
  </div>
  ${renderInlineSectionDescription("recommended-actions")}
  ${emptyBody}
  ...
  <div>
    ...
    <div class="meta">${escapeHtml(item.reason || "-")} · ${renderLicenseTag(item.license)}</div>
  </div>
  ...
  const bodyContent = configs.map(renderGroup).join("\n");
  if (bodyOnly) return `<div class="section-body">${bodyContent}</div>`;
  ...
  <div class="section-head">
    ...
    <div class="head-actions"><span class="count-chip">${totalItems} Kandidaten</span></div>
  </div>
  ${renderInlineSectionDescription("recommended-actions")}
  <div class="section-body">
    ${bodyContent}
