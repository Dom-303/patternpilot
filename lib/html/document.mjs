...
import { INFO_DIALOG_SCRIPT, renderInfoDialog, renderExplainButton } from "./components.mjs";
...
function renderReportContentIntro({ reportType, projectKey, createdAt, heroSubtitle, candidateCount, runRoot }) {
  ...
  const actionsFragment = introInfo
    ? `<div class="intro-actions">${renderExplainButton({ title: introInfo.title, body: introInfo.body, darkVariant: true })}</div>`
    : "";
...
export function renderHtmlDocument({
  ...
  const recommendationsInfo = getSectionInfo("recommendations");
  const recommendationsExplain = recommendationsInfo
    ? renderExplainButton({ title: recommendationsInfo.title, body: recommendationsInfo.body })
    : "";
  const recommendationsPanel = recommendations?.length
    ? `<section class="section-preview accent-magenta" id="recommendations" data-nav-section>
      <div class="section-head">
        ...
        <div class="head-actions">${recommendationsExplain}<span class="count-chip">${recommendations.length} Zeilen</span></div>
      </div>
    ...
    : "";
  const statsInfo = getSectionInfo("stats");
  const statsExplain = statsInfo
    ? renderExplainButton({ title: statsInfo.title, body: statsInfo.body })
    : "";
