...
function renderInlineDescription(sectionId) {
...
function renderReportContentIntro({ reportType, projectKey, createdAt, heroSubtitle, candidateCount, runRoot }) {
  ...
  // Keine Description-Collapse am Content-Intro — User-Feedback: dort
  // uebertrieben, weil der Hero bereits selbsterklaerend ist.
...
export function renderHtmlDocument({
  ...
  const recsEmptyAttr = recsLen === 0 ? ' data-section-empty="true"' : "";
  const recommendationsPanel = `<section class="section-preview accent-magenta" id="recommendations" data-nav-section${recsEmptyAttr}>
      <div class="section-head">
        <div class="title">
          <h2><span class="marker"></span>Erste Empfehlungen</h2>
          <div class="sub">${recsLen > 0 ? "Staerkste Kandidaten aus diesem Lauf" : "Keine Empfehlungen verfuegbar"}</div>
        </div>
        <div class="head-actions"><span class="count-chip">${recsLen} Zeilen</span></div>
      </div>
      ${renderInlineDescription("recommendations")}
      <div class="section-body">
        ${recsLen > 0
          ? renderTopRecommendations(recommendations, candidates)
          : `<p class="empty">Noch keine Empfehlungen vorhanden. Sie erscheinen sobald Discovery-Kandidaten mit Dispositions-Signalen auftauchen oder die Watchlist konkrete Treffer liefert.</p>`}
      </div>
    </section>`;
  const statsPanel = `<section class="section-preview accent-cyan" id="stats" data-nav-section>
      <div class="section-head">
        <div class="title">
          <h2><span class="marker"></span>Kennzahlen</h2>
          <div class="sub">Grundzahlen auf einen Blick</div>
        </div>
        <div class="head-actions"><span class="count-chip">${orderedStats.length} Felder</span></div>
      </div>
      ${renderInlineDescription("stats")}
  ...
  // Alle Overview-Bloecke sind jetzt always-emit (mit Empty-States), deshalb
  // werden sie immer in die Sidenav aufgenommen — Nav und Content bleiben
  // immer synchron.
  const overviewNavItems = [
    { id: "stats", navLabel: "Kennzahlen" },
    { id: "decision-summary", navLabel: "Entscheidungsuebersicht" },
    { id: "recommendations", navLabel: "Erste Empfehlungen" },
    { id: "recommended-actions", navLabel: "Empfohlene Aktionen" }
  ];
  const navSections = [
    ...
    <main class="wrap page" id="top">
      ...
      <div class="sections">
        ${sections.map((section) => {
          // skipSectionWrapper: caller hat die Section bereits mit
          // renderTabbedSection oder equivalent selbst gewrappt.
          if (section.skipSectionWrapper) {
            return section.body;
          }
      ...
      <footer class="report-footer">
        ...
        <nav class="footer-cta" aria-label="Naechste Schritte">
          <a class="footer-cta-link" href="#report-intro">
            <span class="footer-cta-label">Report</span>
            <span class="footer-cta-hint">Zurueck nach oben</span>
          </a>
          <a class="footer-cta-link" href="#recommended-actions">
            <span class="footer-cta-label">Aktionen</span>
            <span class="footer-cta-hint">Was jetzt tun</span>
          </a>
          <a class="footer-cta-link" href="#agent-view">
            <span class="footer-cta-label">Agent Hand-Off</span>
            <span class="footer-cta-hint">Fuer KI-Coding-Agenten</span>
          </a>
          <a class="footer-cta-link" href="#target-repo-context">
            <span class="footer-cta-label">Kontext</span>
            <span class="footer-cta-hint">Datenbasis pruefen</span>
          </a>
        </nav>
        <p class="footer-next-run"><span class="footer-next-run-label">Naechster Lauf:</span> <code>npm run on-demand -- --project ${escapeHtml(projectKey)}</code></p>
      </footer>
    </main>
  </div>
  ${renderInfoDialog({})}
