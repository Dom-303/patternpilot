...
.sidenav {
  ...
  overflow-y: auto;
  /* Horizontal scroll komplett unterbinden: Nav-Labels fliessen innerhalb
     der Spalte um, statt nach rechts rauszuragen und eine Scrollbar zu
     triggern. */
  overflow-x: hidden;
  /* Scrollbar nach LINKS legen: direction-Flip auf dem Container, damit
     der Browser die Scrollbar auf die linke Seite rendert. Inner-Content
     kriegt direction: ltr zurueck, damit Text normal fliesst. */
  direction: rtl;
  /* Filigrane Scrollbar passend zum Cockpit-Night-Theme — schmal, dezent,
     aktiviert sich visuell auf Hover. */
  scrollbar-width: thin;
  scrollbar-color: var(--card-rule) transparent;
}
.sidenav:hover {
...
.sidenav > * {
  /* direction-Flip auf dem Container muss im Inhalt wieder auf ltr gesetzt
     werden, sonst laufen Text und Layout rueckwaerts. */
  direction: ltr;
}
.sidenav-logo-link {
  display: inline-block;
...
.sidenav-list a {
  ...
  text-transform: uppercase;
  line-height: 1.3;
  /* Lange Labels (z.B. "TECHNISCHER LAUF-STATUS") sollen umbrechen statt
     die Spalte breit zu ziehen und horizontale Scrollbar zu triggern. */
