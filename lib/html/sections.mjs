...
// Konsolidierte Empfehlungen-Section: buendelt "Top-Rang" (renderTopRecommendations)
// + "Nach Disposition gruppiert" (renderRecommendedActions) in einer einzigen
// Section mit Tabs. Nutzbar in html-renderer.mjs und Smokes.
export function renderEmpfehlungenSection({
  ...
  const countChip = recsLen > 0 ? `${recsLen} Top · ${candLen} gruppiert` : candLen > 0 ? `${candLen} gruppiert` : "leer";
  return renderTabbedSection({
    id: "empfehlungen",
    title: "Empfehlungen",
    sub: "Top-Rang + Handlungs-Dispositions",
    accent: "magenta",
    countChip,
    tabs: [
      {
      ...
      },
      {
        ...
        body: renderRecommendedActions({ candidates, reportType, runRoot, discovery, review })
      }
    ]
  });
}
