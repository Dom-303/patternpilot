...
export function renderAgentField(agentView) {
  ...
  // Byte-size & Zeilenzahl der JSON-Payload — hilft dem Menschen
  // einzuschaetzen, was er da herunterlaedt.
  const payloadByteSize = Buffer.byteLength(payloadJson, "utf8");
  const payloadSizeLabel = payloadByteSize >= 1024
    ? `${(payloadByteSize / 1024).toFixed(1)} KB`
    : `${payloadByteSize} B`;
  const payloadLineCount = payloadJson.split("\n").length;

  const iconCopy = `<svg class="agent-action-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="M5 1.5A1.5 1.5 0 0 1 6.5 0h6A1.5 1.5 0 0 1 14 1.5v9A1.5 1.5 0 0 1 12.5 12H11v1.5A2.5 2.5 0 0 1 8.5 16h-6A2.5 2.5 0 0 1 0 13.5V6.5A2.5 2.5 0 0 1 2.5 4H4V1.5z" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/></svg>`;
  const iconOpen = `<svg class="agent-action-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="M9 1h6v6M15 1 7.5 8.5M12 9v4.5A1.5 1.5 0 0 1 10.5 15h-8A1.5 1.5 0 0 1 1 13.5v-8A1.5 1.5 0 0 1 2.5 4H7" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  const iconDownload = `<svg class="agent-action-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="M8 1v9m0 0 3.5-3.5M8 10 4.5 6.5M1.5 11v2.5A1.5 1.5 0 0 0 3 15h10a1.5 1.5 0 0 0 1.5-1.5V11" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  const iconFile = `<svg class="agent-file-icon" viewBox="0 0 20 20" aria-hidden="true"><path d="M4 2h7l4 4v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1z" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/><path d="M11 2v4h4" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/><text x="5" y="14.5" font-family="JetBrains Mono, monospace" font-size="4.2" font-weight="700" fill="currentColor" letter-spacing="0.2">JSON</text></svg>`;
  ...
  <div class="agent-snapshot-toolbar">
    <div class="agent-snapshot-file">
      ...
      <div class="agent-snapshot-file-meta">
        <span class="agent-snapshot-file-name" title="${downloadName}">${downloadName}</span>
        <span class="agent-snapshot-file-stats">${payloadSizeLabel} &middot; ${payloadLineCount} Zeilen &middot; stabiles JSON-Schema</span>
      </div>
    </div>
    <div class="agent-snapshot-actions">
      <button type="button" class="agent-action-button" data-agent-action="copy" aria-label="JSON in Zwischenablage kopieren">${iconCopy}<span>Kopieren</span></button>
      <button type="button" class="agent-action-button" data-agent-action="open" aria-label="JSON in neuem Tab oeffnen">${iconOpen}<span>Oeffnen</span></button>
      <button type="button" class="agent-action-button agent-action-button--primary" data-agent-action="download" data-agent-filename="${downloadName}" aria-label="JSON herunterladen">${iconDownload}<span>Herunterladen</span></button>
    </div>
  </div>
  <details class="agent-json">
    <summary>
      <span class="agent-json-chevron" aria-hidden="true">&#x203A;</span>
      <span class="agent-json-label">JSON-Vorschau einblenden</span>
      <span class="agent-json-hint">fuer KI-Agenten relevant &middot; ${payloadLineCount} Zeilen</span>
    </summary>
    <div class="agent-json-body">
      <pre class="agent-pre"><code class="agent-code">${escapeHtml(payloadJson)}</code></pre>
    </div>
  </details>
</div>`;
...
export function renderCoverageCards(coverage, options = {}) {
  // Optionale Labels — damit Landscape "Faehigkeiten" zu "Beziehung zum
  // aktuellen Ansatz" umbenennen kann, ohne dass Discovery die Semantik
  // verliert. Default-Labels entsprechen weiterhin der Discovery-Welt.
  const titles = {
    mainLayers: options.titles?.mainLayers ?? "Hauptlayer",
    gapAreas: options.titles?.gapAreas ?? "Lueckenbereiche",
    capabilities: options.titles?.capabilities ?? "Faehigkeiten"
  };
  const groups = [
    { title: titles.mainLayers, items: coverage.mainLayers ?? [] },
    { title: titles.gapAreas, items: coverage.gapAreas ?? [] },
    { title: titles.capabilities, items: coverage.capabilities ?? [] }
...
// Mapping Cluster-Relation -> Handlungs-Verdikt + konkrete naechste Mini-
// Aktion. Der Verdikt-Satz ist bewusst kurz, damit er in Tab 1 pro Cluster
// gut lesbar bleibt.
function clusterVerdict(relation) {
  if (relation === "divergent") return {
    tone: "adopt",
    label: "Uebernehmen pruefen",
    action: "Lies 1 Top-Repo tief, entscheide ob Pattern ins Zielprojekt gehoert"
  };
  if (relation === "adjacent") return {
    tone: "adapt",
    label: "Adaptions-Skizze",
    action: "Skizziere ein Mini-Spike: welche 1-2 Schnitte braucht die Adaption?"
  };
  return {
    tone: "observe",
    label: "Beobachten",
    action: "Watchlist-Eintrag reicht — kein akuter Handlungsbedarf"
  };
}
...
export function renderLandscapeEntscheidungenSection({ clusters = [], landscapeSignal = "" } = {}) {
  ...
  const countChip = clusters.length > 0 ? `${clusters.length} Cluster` : "leer";
  const relationOrder = ["divergent", "adjacent", "near_current_approach"];

  // Tab 1 "Cluster-Verdikt": pro Cluster ein Handlungs-Urteil mit einer
  // konkreten Naechste-Mini-Aktion. Strategische Ebene — was mache ich mit
  // diesem Cluster als Ganzes, nicht mit einzelnen Repos.
  const sortedClusters = [...clusters].sort((a, b) => {
    const order = { divergent: 0, adjacent: 1, near_current_approach: 2 };
    return (order[a.relation] ?? 3) - (order[b.relation] ?? 3);
  });
  const verdictBody = sortedClusters.length === 0
    ? `<p class="empty">Noch keine Cluster — ohne Cluster kein Handlungsverdikt.</p>`
    : sortedClusters.map((cluster, i) => {
        const verdict = clusterVerdict(cluster.relation);
        const members = cluster.member_ids ?? [];
        const patternFamily = cluster.pattern_family ?? "-";
        return `<div class="repo-row">
  <div>
    <div class="name">${String(i + 1).padStart(2, "0")}. ${escapeHtml(cluster.label ?? "unlabeled")}</div>
    <div class="meta">${escapeHtml(members.length)} Repos · ${escapeHtml(patternFamily)} · Naechster Schritt: ${escapeHtml(verdict.action)}</div>
  </div>
  <span class="badge ${verdict.tone}">${escapeHtml(verdict.label)}</span>
</div>`;
      }).join("\n");

  // Tab 2 "Nach Impact": strategische Bucketing nach Erwartungsnutzen —
  // divergent=hoch, adjacent=mittel, near=niedrig. Im Unterschied zum alten
  // "Nach Relation" stellt diese Sicht Impact in den Vordergrund statt die
  // Taxonomie — das ist was der User fuer seine Prioritaet braucht.
  const impactGroups = { hoch: [], mittel: [], niedrig: [] };
  for (const c of clusters) {
    const bucket = c.relation === "divergent" ? "hoch"
                 : c.relation === "adjacent" ? "mittel"
                 : "niedrig";
    impactGroups[bucket].push(c);
  }
  const impactOrder = ["hoch", "mittel", "niedrig"];
  const impactHints = {
    hoch: "Hoechstes Lern-Potenzial — hier investieren, wenn Zeit da ist",
    mittel: "Adaptionsbereit — hier entstehen schnelle Mini-Spikes",
    niedrig: "Bestaetigt, nicht priorisieren — Watchlist-Modus"
  };
  const byImpactBody = impactOrder.every((b) => impactGroups[b].length === 0)
    ? `<p class="empty">Noch keine Cluster zum Impact-Bucketing.</p>`
    : impactOrder.map((bucket) => {
        const group = impactGroups[bucket];
        if (group.length === 0) return "";
        return `<div class="coverage-axis-group">
  <div class="group-head"><h3>Impact ${escapeHtml(bucket)} · ${group.length} Cluster</h3><span class="group-head-meta">${escapeHtml(impactHints[bucket])}</span></div>
  ${group.map((c) => {
    const verdict = clusterVerdict(c.relation);
    return `<div class="repo-row">
    <div>
      <div class="name">${escapeHtml(c.label ?? "unlabeled")}</div>
      <div class="meta">${escapeHtml(c.pattern_family ?? "-")} · ${escapeHtml(c.member_ids?.length ?? 0)} Repos</div>
    </div>
    <span class="badge ${verdict.tone}">${escapeHtml(verdict.label)}</span>
  </div>`;
  }).join("\n")}
  ...
  return renderTabbedSection({
    ...
    sub: "Cluster-Verdikt · Nach Impact · Strategische Begruendung",
    accent: "magenta",
    countChip,
    tabs: [
      { label: "Cluster-Verdikt", body: verdictBody },
      { label: "Nach Impact", body: byImpactBody },
      { label: "Strategische Begruendung", body: reasoningBody }
    ]
  });
}

// Landscape-spezifische Empfehlungen-Section (3 Tabs), parallel zur
// Entscheidungen-Section. Unterschied:
//   - Entscheidungen = CLUSTER-Ebene, strategisch (welcher Cluster ist
//     wie einzuordnen, Impact pro Cluster)
//   - Empfehlungen  = REPO-Ebene, taktisch (welche konkreten Repos jetzt
//     anschauen, Pro/Contra pro Repo, Disposition adopt/adapt/observe)
// Damit bekommt der Nutzer beide Perspektiven: Landscape-Ueberblick oben,
// konkrete Action-Items hier.
function _landscapeRepoRelationDisposition(relation) {
  if (relation === "divergent") return { tone: "adopt", label: "Uebernehmen pruefen" };
  if (relation === "adjacent") return { tone: "adapt", label: "Adaptieren" };
  return { tone: "observe", label: "Beobachten" };
}

function _landscapeRepoLabel(memberId) {
  const raw = String(memberId ?? "");
  const httpMatch = raw.match(/^https?:\/\/github\.com\/([^/\s]+)\/([^/\s#?]+)/i);
  if (httpMatch) return `${httpMatch[1]}/${httpMatch[2].replace(/\.git$/, "")}`;
  if (/^[^/\s]+\/[^/\s]+$/.test(raw)) return raw;
  return raw;
}

function _landscapeRepoUrl(memberId) {
  const raw = String(memberId ?? "");
  const httpMatch = raw.match(/^https?:\/\/github\.com\/[^/\s]+\/[^/\s#?]+/i);
  if (httpMatch) return httpMatch[0].replace(/\.git$/, "");
  if (/^[^/\s]+\/[^/\s]+$/.test(raw)) return `https://github.com/${raw}`;
  return "";
}

export function renderLandscapeEmpfehlungenSection({ clusters = [], topN = 5 } = {}) {
  const allRepos = [];
  for (const cluster of clusters) {
    ...
    for (const m of members) {
      allRepos.push({
        id: m,
        label: _landscapeRepoLabel(m),
        url: _landscapeRepoUrl(m),
        relation: cluster.relation,
        clusterLabel: cluster.label,
        patternFamily: cluster.pattern_family,
        mainLayer: cluster.main_layer,
        signatureContrast: Array.isArray(cluster.signature_contrast) ? cluster.signature_contrast : []
      });
    }
  }

  const totalRepos = allRepos.length;
  const countChip = totalRepos === 0
    ? "leer"
    : `${Math.min(topN, totalRepos)} Top-Empfehlungen · ${totalRepos} Repos gesamt`;

  // Sortier-Prioritaet: divergent > adjacent > near_current_approach,
  // innerhalb derselben Relation: groessere Cluster (mehr Evidenz) zuerst.
  const relationPriority = { divergent: 0, adjacent: 1, near_current_approach: 2 };
  const repoSizeByCluster = new Map();
  clusters.forEach((c) => repoSizeByCluster.set(c.label, (c.member_ids ?? []).length));
  const sortedAll = [...allRepos].sort((a, b) => {
    const relDiff = (relationPriority[a.relation] ?? 3) - (relationPriority[b.relation] ?? 3);
    if (relDiff !== 0) return relDiff;
    return (repoSizeByCluster.get(b.clusterLabel) ?? 0) - (repoSizeByCluster.get(a.clusterLabel) ?? 0);
  });

  // --- Tab 1: Top-Rang (kurz) ------------------------------------------------
  const topPicks = sortedAll.slice(0, topN);
  const topRangBody = topPicks.length === 0
    ? `<p class="empty">Noch keine Repos im Landscape — keine Empfehlungen ableitbar. Sobald Cluster Mitglieder haben, erscheinen hier die Top-Kandidaten zum Vertiefen.</p>`
    : topPicks.map((repo, i) => {
        const disposition = _landscapeRepoRelationDisposition(repo.relation);
        const meta = [repo.clusterLabel, repo.patternFamily].filter(Boolean).join(" · ") || "-";
        const nameFragment = repo.url
          ? `<a class="name" href="${escapeHtml(repo.url)}" target="_blank" rel="noopener noreferrer">${String(i + 1).padStart(2, "0")}. ${escapeHtml(repo.label)}</a>`
          : `<div class="name">${String(i + 1).padStart(2, "0")}. ${escapeHtml(repo.label)}</div>`;
        return `<div class="repo-row">
  <div>
    ${nameFragment}
    <div class="meta">${escapeHtml(meta)}</div>
  </div>
  <span class="badge ${disposition.tone}">${escapeHtml(disposition.label)}</span>
</div>`;
      }).join("\n");

  // --- Tab 2: Nach Disposition gruppiert ------------------------------------
  const buckets = { divergent: [], adjacent: [], near_current_approach: [] };
  for (const repo of sortedAll) {
    if (buckets[repo.relation]) buckets[repo.relation].push(repo);
  }
  const bucketLabels = {
    divergent: { title: "Uebernehmen pruefen — divergente Ansaetze", hint: "Neue Denkweise, hohes Lern-Potenzial. Transfer aufwendig, aber strategisch wertvoll.", tone: "adopt" },
    adjacent: { title: "Adaptieren — benachbarte Ansaetze", hint: "Verwandt, wenige Schnitte. Konkrete Adaptions-Skizze je Repo lohnt.", tone: "adapt" },
    near_current_approach: { title: "Beobachten — nahe am aktuellen Ansatz", hint: "Bestaetigt den Weg, wenig Neu-Sicht. Watchlist reicht meistens.", tone: "observe" }
  };
  const relationOrder = ["divergent", "adjacent", "near_current_approach"];
  const anyBucketFilled = relationOrder.some((r) => buckets[r].length > 0);
  const byDispositionBody = !anyBucketFilled
    ? `<p class="empty">Noch keine Repos zum Gruppieren vorhanden.</p>`
    : relationOrder.map((rel) => {
        const group = buckets[rel];
        if (group.length === 0) return "";
        const cfg = bucketLabels[rel];
        const rows = group.slice(0, 10).map((repo) => {
          const nameFragment = repo.url
            ? `<a class="name" href="${escapeHtml(repo.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(repo.label)}</a>`
            : `<div class="name">${escapeHtml(repo.label)}</div>`;
          return `<div class="repo-row">
  <div>
    ${nameFragment}
    <div class="meta">${escapeHtml(repo.clusterLabel)}${repo.patternFamily ? ` · ${escapeHtml(repo.patternFamily)}` : ""}</div>
  </div>
  <span class="badge ${cfg.tone}">${escapeHtml(cfg.title.split(" — ")[0])}</span>
</div>`;
        }).join("\n");
        const more = group.length > 10 ? `<div class="meta" style="margin-top:8px">... und ${group.length - 10} weitere</div>` : "";
        return `<div class="coverage-axis-group">
  <div class="group-head"><h3>${escapeHtml(cfg.title)} · ${group.length}</h3><span class="group-head-meta">${escapeHtml(cfg.hint)}</span></div>
  ${rows}
  ${more}
</div>`;
      }).filter(Boolean).join("\n");

  // --- Tab 3: Entscheidungs-Begruendung (pro Top-Repo Pro/Contra + Impact) --
  const reasoningBody = topPicks.length === 0
    ? `<p class="empty">Noch keine Top-Empfehlungen mit Begruendung ableitbar.</p>`
    : `<div class="decision-reasoning-list">${topPicks.map((repo, i) => {
        const disposition = _landscapeRepoRelationDisposition(repo.relation);
        const impact = repo.relation === "divergent" ? "hoch" : repo.relation === "adjacent" ? "mittel" : "niedrig";
        const pros = [];
        const contras = [];
        if (repo.relation === "divergent") {
          pros.push(`Divergenter Ansatz in "${repo.clusterLabel}" — echte Neu-Sicht moeglich`);
          pros.push("Hoher Erkenntnisgewinn beim Durchlesen des Repos");
          if (repo.signatureContrast.length > 0) pros.push(`Unterscheidende Signale: ${repo.signatureContrast.slice(0, 3).join(", ")}`);
          contras.push("Transfer ins Zielprojekt aufwendig — Adaption statt Copy-Paste");
          contras.push("Darauf achten, nicht unkritisch das ganze Paradigma zu uebernehmen");
        } else if (repo.relation === "adjacent") {
          pros.push(`Benachbart zu bestehendem Ansatz — niedrige Transfer-Huerde`);
          pros.push(`Konkrete Adaptions-Skizze aus Cluster "${repo.clusterLabel}" machbar`);
          if (repo.patternFamily) pros.push(`Musterfamilie "${repo.patternFamily}" gibt klaren Rahmen`);
          contras.push("Weniger Neu-Sicht als divergente Cluster");
          contras.push("Inkrementelle Verbesserung — nicht zwingend strategischer Hebel");
        } else {
          pros.push("Bestaetigt tragfaehigen Weg — geringe Transfer-Huerde");
          pros.push("Kann fuer Selbstvergleich / Benchmarking dienen");
          contras.push("Wenig neue Einsicht — kein Haupt-Investitions-Hebel");
          contras.push("Watchlist reicht meistens, volle Intake-Ueberfuehrung selten noetig");
        }
        const nextRepo = topPicks[i + 1];
        const altFragment = nextRepo
          ? `<div class="decision-reasoning-alt">Naechste Empfehlung: <strong>${escapeHtml(nextRepo.label)}</strong></div>`
          : "";
        return `<div class="decision-reasoning-item">
  <div class="decision-reasoning-head">
    <span class="decision-reasoning-rank">${String(i + 1).padStart(2, "0")}</span>
    ${repo.url
      ? `<a class="decision-reasoning-name" href="${escapeHtml(repo.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(repo.label)}</a>`
      : `<span class="decision-reasoning-name">${escapeHtml(repo.label)}</span>`}
    <span class="badge ${disposition.tone}">${escapeHtml(disposition.label)}</span>
    <span class="decision-reasoning-impact impact-${impact}">Impact ${escapeHtml(impact)}</span>
  </div>
  <div class="decision-reasoning-body">
  ...
  return renderTabbedSection({
    id: "empfehlungen",
    title: "Empfehlungen",
    sub: "Top-Rang · Nach Disposition · Begruendung",
    accent: "magenta",
    countChip,
    tabs: [
      { label: "Top-Rang (kurz)", body: topRangBody },
      { label: "Nach Disposition gruppiert", body: byDispositionBody },
