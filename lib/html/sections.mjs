...
export function renderDiscoveryCandidateCards(candidates, reportView) {
  ...
  return `<div class="repo-grid">${visible.map((candidate) => {
    const whyRelevant = localizeGeneratedText(candidate.reasoning[0] ?? "Braucht eine manuelle Pruefung.");
    ...
    const strengths = localizeGeneratedText(candidate.landkarteCandidate?.strengths ?? "-");
    const risks = localizeGeneratedText(candidate.landkarteCandidate?.risks ?? "manuell pruefen");
  ...
  <div class="repo-head">
    ...
    <div class="repo-badges">
      ...
      ${candidate.discoveryEvidence?.grade ? renderBadge(`Evidenz ${candidate.discoveryEvidence.grade}`, "neutral") : ""}
      ${candidate.discoveryClass ? renderBadge(candidate.discoveryClass.replace(/_/g, " "), "neutral") : ""}
    </div>
  </div>
  ...
  <p class="repo-copy">${escapeHtml(candidate.enrichment?.repo?.description || "Keine oeffentliche Beschreibung vorhanden.")}</p>
  <dl class="mini-grid">
    <div><dt>Warum relevant</dt><dd>${escapeHtml(whyRelevant)}</dd></div>
    <div><dt>Evidenz</dt><dd>${escapeHtml(candidate.discoveryEvidence ? `${candidate.discoveryEvidence.grade} (${candidate.discoveryEvidence.score})` : "-")}</dd></div>
...
export function renderReportToolbar({ modeOptions, layerOptions }) {
  return `<section class="toolbar-card section-card" id="report-toolbar">
  <header class="section-head">
    <div>
      <p class="panel-kicker">Werkzeugbereich</p>
      <h2>Bericht filtern</h2>
      <p class="panel-copy">Nutze diesen Bereich nur dann, wenn der Bericht viele Karten hat. Fuer kurze Laeufe kannst du ihn meist ignorieren.</p>
    </div>
  </header>
  ...
  <div class="toolbar-grid">
    <label class="control">
      <span>Suche</span>
      <input id="report-search" type="search" placeholder="Repo, Thema oder Ebene suchen">
    </label>
    <label class="control">
      <span>Passung</span>
    ...
    <button class="ghost-button" id="report-reset" type="button">Zuruecksetzen</button>
  </div>
  <p class="toolbar-help">Der Filter ist ein Hilfswerkzeug fuer lange Berichte. Er soll den Bericht nicht fuehren, sondern nur schneller zum passenden Ausschnitt bringen.</p>
  ${renderFilterIndicator()}
  </div>
</section>`;
}
