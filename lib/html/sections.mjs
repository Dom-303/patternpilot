...
function renderRepoBodyListCell(key, items, fallback) {
  const list = (items ?? []).filter(Boolean);
  const body = list.length > 0
    ? `<ul class="repo-body-list">${list.map((it) => `<li>${escapeHtml(it)}</li>`).join("")}</ul>`
    : `<div class="repo-body-value">${escapeHtml(fallback)}</div>`;
...
export function renderDiscoveryCandidateCards(candidates, reportView) {
  ...
  return visible.map((candidate) => {
    ...
    <div class="repo-body-grid">
      ${renderRepoBodyCell("Warum relevant", whyRelevant, { wide: true })}
      ${renderRepoBodyCell("Evidenz", evidenceText)}
      ${renderRepoBodyCell("Staerke", strengths)}
      ${renderRepoBodyCell("Transferidee", transfer)}
      ${renderRepoBodyCell("Risiken", risks, { wide: true })}
      ${renderRepoBodyListCell("Begruendung", reasoningItems, "Keine Begruendung erfasst.")}
    </div>
  </div>`;
...
export function renderWatchlistTopCards(review, reportView) {
  ...
  return visible.map((item) => {
    ...
    const body = `<div class="repo-body">
    <div class="repo-body-grid">
      ${renderRepoBodyCell("Warum wichtig", whyImportant, { wide: true })}
      ${renderRepoBodyCell("Was du mitnehmen kannst", takeaway, { wide: true })}
      ${renderRepoBodyCell("Staerke", strengths)}
      ${renderRepoBodyCell("Schwaeche / Risiko", weaknesses)}
      ${renderRepoBodyListCell("Vergleichsdetails", detailsItems, "Keine Details vorhanden.")}
    </div>
  </div>`;
...
export function renderAgentField(agentView) {
  ...
  const secondaryStarterFragments = secondaryStarters
    .map((starter, i) => renderAgentStarterCard(starter, { title: `Sekundaerer Pfad ${i + 1}` }))
    .join("\n");
