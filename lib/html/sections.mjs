...
export function renderDiscoveryCandidateCards(candidates, reportView) {
  ...
  <div class="repo-head">
    ...
    <div class="repo-badges">
      ...
      ${candidate.discoveryEvidence?.grade ? renderBadge(`Evidence ${candidate.discoveryEvidence.grade}`, "neutral") : ""}
  ...
  <dl class="mini-grid">
    ...
    <div><dt>Evidence</dt><dd>${escapeHtml(candidate.discoveryEvidence ? `${candidate.discoveryEvidence.grade} (${candidate.discoveryEvidence.score})` : "-")}</dd></div>
...
export function renderRepoMatrix(review, reportView) {
  ...
  <tbody>
    ${rows.map((item) => `<tr>
      ...
      <td>${escapeHtml(getRenderedProjectRelevance(item, "-"))}</td>
