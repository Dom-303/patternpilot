...
export function renderCoverageCards(coverage) {
  ...
  const intro = `<p class="section-intro">Coverage zeigt, wie sich die Kandidaten ueber ${termHtml("fit-band", { label: "Haupt-Layer" })}, ${termHtml("gapArea", { label: "Luecken-Bereiche" })} und ${termHtml("matched-capabilities", { label: "Faehigkeiten" })} verteilen. Die Balken sind <strong>relativ zum Maximum der jeweiligen Gruppe</strong>, nicht absolut — ein 100%-Balken heisst „Spitzenwert dieser Gruppe", nicht „vollstaendig abgedeckt".</p>`;
...
export function renderProjectContextSources(projectProfile, binding) {
  ...
  const intro = `<p class="section-intro">Der Zielrepo-Kontext zeigt, welche Dateien und Verzeichnisse Pattern Pilot aus dem Zielprojekt gelesen hat, bevor es Kandidaten bewertet. Damit kannst du nachvollziehen, auf welcher Informationsbasis die Empfehlungen stehen. Die vier Karten unten decken vier Blickwinkel ab: <strong>was gelesen wurde</strong>, <strong>was konfiguriert war aber fehlte</strong>, <strong>welche Verzeichnisse gescannt wurden</strong> und <strong>welche Faehigkeiten daraus abgeleitet sind</strong>.</p>`;

  // Status-Chip: wieviel Pflicht-Material tatsaechlich gelesen wurde
  ...
  const grid = renderInfoGrid([
    {
      title: "Zuerst gelesene Dateien",
      copy: "Konkrete Dateipfade, aus denen der Projektkontext extrahiert wurde. Die Reihenfolge entspricht der read-Priority der Projekt-Bindung.",
      items: loadedFiles.length > 0 ? loadedFiles : declaredFiles,
      emptyText: "Keine Dateien gelesen. Grund: Fuer dieses Ziel-Repository sind keine Kontextdateien in der Bindung konfiguriert (siehe PROJECT_BINDING.json)."
    },
    {
      title: "Fehlende konfigurierte Dateien",
      copy: "Dateien, die laut Bindung gelesen werden sollten, aber am Zielrepo nicht gefunden wurden. Sollte idealerweise leer sein.",
      items: missingFiles,
      emptyText: "Keine fehlenden Dateien. Alle konfigurierten Kontextdateien waren beim Scan verfuegbar."
    },
    {
      title: "Durchsuchte Verzeichnisse",
      copy: "Verzeichnisse, in denen die Struktur (Dateinamen, grobe Module) gesampelt wurde. Zahl in Klammern = Anzahl gelesener Eintraege.",
      ...
      emptyText: "Keine Verzeichnisse gescannt. Grund: Es ist kein Verzeichniskontext in der Bindung konfiguriert."
    },
    {
      title: "Extrahierte Signale",
      copy: "Zielprojekt-Faehigkeiten (capabilities), die Pattern Pilot aus dem Kontext abgeleitet hat. Diese fliessen direkt ins matched-capabilities-Ranking der Kandidaten.",
      items: capabilitiesPresent.map((item) => `Faehigkeit: ${localizeSystemTerm(item)}`),
      emptyText: "Keine Signale abgeleitet. Grund: Aus dem aktuellen Kontext konnten keine strukturierten Projekt-Faehigkeiten extrahiert werden — entweder sind die Kontextquellen zu duenn oder der Capability-Detector hat keine Muster erkannt."
    }
  ], { variant: "halves" });

  return `${intro}${statusChip}${grid}`;
}
