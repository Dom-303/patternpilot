...
export function renderSidenav({ logoSrc, logoAlt = "Pattern Pilot", eyebrow = "Inhalt", items = [] } = {}) {
  ...
  const links = items
    .map((item, index) => {
      ...
      return `    <li><a href="${escapeHtml(href)}"${activeCls}${activeAttr}><span class="n">${number}</span><span class="label">${label}</span></a></li>`;
    })
    .join("\n");
...
// Agent-Snapshot-Script: behandelt Copy / Open / Download fuer die
// Maschinenlesbares-Snapshot-Karte. Liest die JSON-Payload entweder aus
// dem <script id="patternpilot-agent-payload"> oder — als Fallback — aus
// dem sichtbaren <pre class="agent-pre">. Identisch nutzbar in Main-
// Report und Landscape-Report.
export const AGENT_SNAPSHOT_SCRIPT = `(() => {
  ...
  const createAgentBlobUrl = () => {
    ...
    return URL.createObjectURL(new Blob([stringifyPayload(payload)], { type: "application/json" }));
  };
  ...
  const flashCopied = (button) => {
    ...
    setTimeout(() => {
      ...
      if (label && originalText !== undefined) label.textContent = originalText;
    }, 1600);
  };
    ...
    button.addEventListener("click", async () => {
      ...
      if (action === "copy") {
        ...
        try {
          ...
          console.warn("[patternpilot] copy fehlgeschlagen", err);
        }
        return;
      }
