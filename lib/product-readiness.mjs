import { describeAutomationOperatingMode } from "./automation/operating-mode.mjs";
...
function buildAutomationJobsCheck(automation = {}, automationMode = describeAutomationOperatingMode(automation)) {
  if ((automation.jobsConfigured ?? 0) <= 0) {
    return buildCheck({
      key: "automation_jobs",
      label: "Automation operating mode",
      status: "pass",
      detail: automationMode.summary
    });
  }
  ...
  return buildCheck({
    key: "automation_jobs",
    label: "Automation operating mode",
    status: automationMode.modeKey === "operator_attention_required" ? "followup" : "pass",
    detail: `${automationMode.summary} Configured jobs=${automation.jobsConfigured ?? 0}; ready=${automation.jobsReady ?? 0}; blocked=${automation.jobsBlocked ?? 0}; backoff=${automation.jobsBackoff ?? 0}.`,
    nextAction: automationMode.modeKey === "operator_attention_required" ? automation.nextAction ?? automation.fallbackNextAction ?? null : null
  });
}

function buildAlertDeliveryCheck(alertDelivery = {}, automation = {}, automationMode = describeAutomationOperatingMode(automation)) {
  if (automationMode.modeKey === "core_only") {
    return buildCheck({
      key: "alert_delivery",
      label: "Alert delivery surface",
      status: "pass",
      detail: "Alert delivery is intentionally optional while automation is not configured."
    });
  }
...
function buildAutomationAttentionCheck(automation = {}, generatedAt, recentCommandIndex, automationMode = describeAutomationOperatingMode(automation)) {
  ...
  if (automationMode.modeKey === "core_only") {
    return buildCheck({
      ...
      status: "pass",
      detail: "No automation attention surface is active because the product is currently running in local core mode."
    });
  }
...
function buildAutomationJobCheck(project = {}, generatedAt, recentCommandIndex) {
  ...
  if (status === "unconfigured") {
    return buildCheck({
      ...
      status: "pass",
      detail: "No project-specific automation job is configured, which is acceptable while operating in the local core workflow."
    });
  }
...
export function buildPatternpilotProductReadinessReview(context = {}) {
  const automationMode = describeAutomationOperatingMode(context.automation);
  ...
  const globalChecks = [
    ...
    buildAutomationJobsCheck(context.automation, automationMode),
    buildAlertDeliveryCheck(context.alertDelivery, context.automation, automationMode),
    buildAutomationAttentionCheck(context.automation, context.generatedAt, recentCommandIndex, automationMode)
  ];
  ...
  return {
    ...
    automationMode: automationMode.modeKey,
    automationOperatorMode: automationMode.operatorMode,
    automationSummary: automationMode.summary
  };
}
...
- automation_mode: ${review.automationMode ?? "core_only"}
- automation_operator_mode: ${review.automationOperatorMode ?? "manual_first"}
...
- summary: ${review.automationSummary ?? "Patternpilot is operating in its local core mode."}
