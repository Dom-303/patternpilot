...
const DISCOVERY_PROFILE_ALIASES = {
  quick: "focused",
  standard: "balanced",
  deep: "expansive",
  focused: "focused",
...
const DISCOVERY_PROFILE_PUBLIC_IDS = {
  focused: "quick",
  balanced: "standard",
  expansive: "deep",
  max: "max"
...
export function resolveDiscoveryProfile(profileName = "balanced", requestedLimit = null) {
  const requestedProfileName = String(profileName ?? "balanced").toLowerCase();
  const internalProfileId = DISCOVERY_PROFILE_ALIASES[requestedProfileName] ?? "balanced";
  const profile = DISCOVERY_PROFILES[internalProfileId] ?? DISCOVERY_PROFILES.balanced;
  ...
  return {
    ...
    publicId: DISCOVERY_PROFILE_PUBLIC_IDS[internalProfileId] ?? internalProfileId,
    ...profile,
    limit
  };
}
