...
- settled_now_nine: first-run onboarding wizard (\`patternpilot init\`) replaces the static getting-started text-print with a 5-step interactive setup (target, context, GitHub auth, discovery profile, optional first action) and resolves OQ-006.
- settled_now_ten: stale-data banner now surfaces drift count and refresh command at the start of intake and on-demand runs; re-evaluate audit log lives in state/re-evaluate-history.json. Resolves OQ-007.
- settled_now_eleven: webhook channel type now sits alongside file/github-summary/command/stdout for Slack/Discord/Teams alerts; auto-resume releases jobs locked longer than 6h (configurable per job). Resolves OQ-008.
- settled_now_twelve: GitHub-App cutover explicitly declined — Patternpilot stays a local-first solo/team tool. Reopen OQ-004 only if a multi-user product positioning emerges that justifies the operational overhead (server, webhook endpoint, permission re-approval flow).
- settled_now_thirteen: Cockpit-Night report UI direction declined-without-action — the existing technical look is fit for purpose at current usage. Reopen OQ-001 only if reports are shown to external stakeholders.
- next_recommended_step: Patternpilot is feature-complete for the local-tool positioning. Future work is observation-driven: tune discovery defaults from real runs, fix issues as they surface, react to changes in the EventBear-Worker target if Patternpilot needs to support new analysis dimensions.
