...
export function buildPolicyWorkbenchReview({ rows = [], sourceRecord = null, currentPolicy = null, proposedPolicy = null }) {
  ...
  const decisionStatus =
    rows.length === 0 ? "not_ready"
      : rowsWithVerdict.length === 0 ? "needs_manual_verdicts"
        : falseBlockCount > 0 || noiseVisibleCount > 0 ? "needs_policy_adjustment"
...
export function renderPolicyWorkbenchReviewSummary({
  ...
  const comparisonLines = review.comparison
    ...
    : "- none";
  const nextCommand =
    review.decisionStatus === "needs_manual_verdicts"
      ? `Edit rows.json in the workbench, then run ${buildWorkbenchCommand(projectKey, workbenchId, "policy-workbench-review")}`
      : buildWorkbenchCommand(projectKey, workbenchId, "policy-suggest");
