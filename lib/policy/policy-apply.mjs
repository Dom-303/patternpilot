...
import { defaultDiscoveryPolicy, validateDiscoveryPolicyShape } from "./discovery-policy.mjs";
...
export async function applyProjectPolicy({
  ...
  const validation = validateDiscoveryPolicyShape(nextPolicy);
  if (!validation.valid) {
    throw new Error(`Cannot apply invalid discovery policy: ${validation.errors.join("; ")}`);
  }
  const changed = currentRaw.trim() !== nextRaw.trim();
  const applyStatus = dryRun
    ? (changed ? "dry_run_apply" : "dry_run_no_change")
    : (changed ? "applied" : "no_change");
  const nextCommand = `npm run patternpilot -- policy-calibrate --project ${projectKey} --limit 5`;
