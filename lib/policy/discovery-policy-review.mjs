...
function summarizeBlockedCandidate(candidate) {
  return {
    ...
    fitScore: candidate?.projectAlignment?.fitScore ?? candidate?.projectFitScore ?? 0,
    ...
    preferenceHits: candidate?.discoveryPolicyGate?.preferenceHits ?? []
  };
}
...
export function buildDiscoveryPolicyReview(discovery, discoveryPolicy) {
  ...
  const auditBlocked = audit.blockedCandidates.map(summarizeBlockedCandidate);
  const hiddenByEnforce = enforce.blockedCandidates.map(summarizeBlockedCandidate);
