...
function buildWorkbenchCommand(projectKey, workbenchId, commandName, extraArgs = "") {
  const suffix = extraArgs ? ` ${extraArgs}` : "";
  return `npm run patternpilot -- ${commandName} --project ${projectKey} --workbench-dir projects/${projectKey}/calibration/workbench/${workbenchId}${suffix}`;
}
...
export function buildPolicyTrial({ discovery, currentPolicy, trialPolicy, sourceRecord = null }) {
  ...
  const decisionStatus =
    sourceCandidates.length === 0 ? "not_ready"
      : newlyVisible.length > 0 && newlyHidden.length === 0 ? "apply_ready"
        : newlyVisible.length > 0 && newlyHidden.length > 0 ? "mixed_review"
          : changedRows.length === 0 ? "no_effect"
            : "review_required";
...
export function renderPolicyTrialSummary({
  ...
  const comparisonLines = trial.comparison
    ...
    : "- none";
  const nextCommand =
    trial.decisionStatus === "apply_ready"
      ? buildWorkbenchCommand(projectKey, workbenchId, "policy-apply", "--policy-file projects/" + projectKey + "/calibration/workbench/" + workbenchId + "/suggested-policy.json")
...
- decision_status: ${trial.decisionStatus}
