...
function buildCurationScore(row) {
  ...
  if ((row.eventbaer_relevance_guess ?? "") === "high") {
    score += 12;
  } else if ((row.eventbaer_relevance_guess ?? "") === "medium") {
    score += 6;
  }
...
export function buildPolicyCuration({ handoffManifest, queueRows = [], limit = null }) {
  ...
  const candidates = queueRows
    ...
    .map((row) => ({
      ...
      relevance: row.eventbaer_relevance_guess || "-",
