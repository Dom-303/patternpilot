import { getProjectRelevanceGuess } from "../legacy-project-fields.mjs";
...
function buildCurationScore(row) {
  ...
  if (getProjectRelevanceGuess(row, "") === "high") {
    score += 12;
  } else if (getProjectRelevanceGuess(row, "") === "medium") {
    score += 6;
  }
...
export function buildPolicyCuration({ projectKey = "project", handoffManifest, queueRows = [], limit = null }) {
  ...
  const candidates = queueRows
    ...
    .map((row) => ({
      ...
      relevance: getProjectRelevanceGuess(row, "-"),
