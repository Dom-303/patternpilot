...
function renderDecisionSummary({ candidates, reportType }) {
  ...
  if (reportType === "discovery") {
    ...
    if (topFit === "high") {
      recommendedMove = `Adopt ${topName} \u2014 strong fit, prioritize integration`;
    } else if (topFit === "medium") {
      recommendedMove = `Study ${topName} \u2014 promising but needs deeper review`;
    } else if (topFit === "low") {
      recommendedMove = `Watch ${topName} \u2014 weak fit, monitor changes`;
    } else {
      recommendedMove = "No strong match \u2014 review top candidates manually";
    }
  }
  ...
  <div class="section-body">
    <div class="summary-grid">
      <div class="summary-item recommended-move">
        ...
        <span class="summary-value">${escapeHtml(recommendedMove)}</span>
      </div>
      ...
      <div class="summary-item">
        <span class="summary-label">Signal confidence <em class="summary-heuristic-label">(heuristic)</em></span>
        <span class="summary-value">${renderBadge(confidence, confidenceTone)} <span class="summary-hint">${confidenceReason}</span></span>
      </div>
      <div class="summary-item">
...
function renderRecommendedActions(candidates, reportType) {
  ...
  <div class="section-body">
    <div class="actions-grid">${configs.map((g) => `<div class="action-group" style="--group-color:${g.color}">
      ...
      ${g.items.map((item, idx) => `<a href="#repo-${item.slug}" class="action-item${g.key === "adopt" && idx < 3 ? " ranked" : ""}">
