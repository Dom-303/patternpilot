...
export function updateAutomationJobState(state, update) {
  ...
  const resumeAutoResumeAllowed = resumeRecommendation?.autoResumeAllowed ?? (resumeRecommendation?.strategy === "retry_after_backoff");
  const autoResumeAllowed = retryableFailures.length > 0 && Boolean(resumeAutoResumeAllowed);
  const recommendedDelayMinutes = autoResumeAllowed
    ? Math.max(...retryableFailures.map((failure) => Number(failure.recommendedDelayMinutes ?? 0) || 0))
    : null;
  const blockedManual = blockingFailures.length > 0 || (retryableFailures.length > 0 && !autoResumeAllowed);
  const failureRecoveryMode = blockedManual
  ...
  next.jobs[update.jobName] = {
    ...
    retryable: autoResumeAllowed && !blockedManual,
    autoResumeAllowed,
    failureRecoveryMode,
    jobScope: update.jobMetadata?.scope ?? previous.jobScope ?? null,
    schedulerHook: update.jobMetadata?.schedulerHook ?? previous.schedulerHook ?? null,
    maxProjectsPerRun: update.jobMetadata?.maxProjectsPerRun ?? previous.maxProjectsPerRun ?? null,
    totalProjects: update.jobMetadata?.totalProjects ?? previous.totalProjects ?? null,
    projectWindowKeys: update.jobMetadata?.projectWindowKeys ?? previous.projectWindowKeys ?? [],
    projectWindowTruncated: update.jobMetadata?.projectWindowTruncated ?? previous.projectWindowTruncated ?? false,
    projectWindowStartCursor: update.jobMetadata?.projectWindowStartCursor ?? previous.projectWindowStartCursor ?? 0,
    nextProjectCursor: update.jobMetadata?.nextProjectCursor ?? previous.nextProjectCursor ?? 0,
...
export function buildAutomationAlerts(evaluations, options = {}) {
  ...
  for (const job of evaluations) {
    ...
    if (job.status === "backoff" && jobState.nextRetryAt) {
      alerts.push({
        severity: "low",
        category: "retry_backoff_scheduled",
        jobName: job.name,
        message: `${job.name} will auto-resume after cooldown at ${jobState.nextRetryAt}.`,
        nextAction: jobState.resumeRecommendation?.nextAction ?? "Wait for the cooldown to elapse; no manual clear is needed unless the same retryable failure repeats."
      });
    }
  ...
  return {
    ...
    nextJob: nextJob
        ? {
          ...
          schedulerHook: nextJob.jobState?.schedulerHook ?? null,
          maxProjectsPerRun: nextJob.jobState?.maxProjectsPerRun ?? null,
          projectWindowKeys: nextJob.jobState?.projectWindowKeys ?? [],
