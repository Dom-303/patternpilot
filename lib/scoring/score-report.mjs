...
function landscapeLabelFidelity(landscape) {
  // Pro Cluster: fuer jedes Label-Token, in wievielen DISTINCT Members
  // dieses Token im Member-Token-Pool (keywords + topics + description)
  // auftaucht. Mittel ueber alle Label-Tokens = Member-Coverage des Labels.
  // Hoehere Coverage = das Label beschreibt wirklich die Mehrheit der
  // Cluster-Members, nicht nur 1-2 Outlier.
  const clusters = Array.isArray(landscape?.clusters) ? landscape.clusters : [];
  ...
  for (const cluster of clusters) {
    ...
    const memberIds = Array.isArray(cluster?.member_ids) ? cluster.member_ids : [];
    const memberTokenSets = [];
    for (const id of memberIds) {
      const member = memberById.get(id);
      if (!member) continue;
      const tokens = new Set();
      if (typeof member.description === 'string') {
        for (const t of tokenize(member.description)) tokens.add(t);
      }
      const kwIterable = member.keywords instanceof Set
        ? member.keywords
        : Array.isArray(member.keywords) ? member.keywords : [];
      for (const kw of kwIterable) {
      ...
      if (Array.isArray(member.topics)) {
        for (const topic of member.topics) {
          ...
          for (const t of tokenize(topic)) tokens.add(t);
        }
      }
      memberTokenSets.push(tokens);
    }
    if (memberTokenSets.length === 0) continue;
    let perTokenCoverageSum = 0;
    for (const labelToken of labelTokens) {
      const containing = memberTokenSets.filter((set) => set.has(labelToken)).length;
      perTokenCoverageSum += containing / memberTokenSets.length;
    }
    const coverage = perTokenCoverageSum / labelTokens.size;
  ...
  const measured = {
    ...
    mean_label_token_member_coverage: Number(meanCoverage.toFixed(3)),
    per_cluster_coverage: perClusterCoverage.map((v) => Number(v.toFixed(3))),
  };
  // Schwellen kalibriert an den realen Cross-Project-Runs:
  // ≥ 0.60 → 2 (alle Label-Tokens in ≥60 % der Members),
  // ≥ 0.30 → 1, sonst 0.
  if (meanCoverage >= 0.60) return buildAxis(2, measured);
  if (meanCoverage >= 0.30) return buildAxis(1, measured);
  return buildAxis(0, measured);
}
