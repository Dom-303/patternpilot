...
function buildGenericBinding({ projectKey, projectLabel, projectRoot, detected }) {
  ...
  const referenceRoot = detected.referenceDirectories.includes("docs") ? "docs" : detected.referenceDirectories[0] ?? ".";
...
- Primaere Referenzoberflaeche: \`${detected.referenceDirectories.includes("docs") ? "docs/" : `${detected.referenceDirectories[0] ?? "."}/`}\`
...
- Primaere Referenzoberflaeche: \`${binding.referenceRoot ?? binding.referenceDirectories[0] ?? "."}/\`
...
${projectSnapshots.map((project) => `- ${project.projectKey}: reference_root=${project.referenceRoot} | watchlist=${project.watchlistCount} | intake_root=${project.intakeRoot} | promotion_root=${project.promotionRoot} | review_root=${project.reviewRoot} | report_root=${project.reportRoot}`).join("\n")}
    ...
    if (bindingPath) {
      ...
      if (bindingContent) {
        try {
          ...
          referenceRoot = binding.referenceRoot ?? binding.referenceDirectories?.[0] ?? "-";
        } catch {
          referenceRoot = "-";
        }
      }
...
export async function loadProjectProfile(rootDir, project, binding, alignmentRules = null) {
  ...
  const referenceDirectories = [];
  const corpusParts = [];
  const referenceRoot = binding.referenceRoot ?? binding.referenceDirectories?.[0] ?? ".";
...
- primary_reference_root: \`${projectProfile?.referenceRoot ?? binding.referenceRoot ?? binding.referenceDirectories[0] ?? "."}\`
