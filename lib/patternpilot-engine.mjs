...
const CATEGORY_RULES = [
  { match: /(wordpress|wp-|plugin)/i, value: "plugin" },
  { match: /(aggregator|calendar|compiled|portal|hub)/i, value: "aggregator" },
  { match: /(place|maps|geo|geocode|venue|location)/i, value: "enricher" },
  { match: /(framework|sdk|infra)/i, value: "framework" },
  { match: /(ui|frontend|discovery|site|web)/i, value: "product_surface" },
  { match: /(scraper|crawler|connector|adapter)/i, value: "connector" }
];

const PATTERN_RULES = [
  { match: /(wordpress|wp-|plugin)/i, value: "cms_distribution_plugin" },
  { match: /(place|maps|geo|venue|location)/i, value: "place_data_infrastructure" },
  { match: /(aggregator|compiled|calendar|events-hub|portal)/i, value: "local_multi_source_aggregator" },
  { match: /(framework|city-|infra|sdk)/i, value: "local_source_infra_framework" },
  { match: /(frontend|discovery|site|web)/i, value: "event_discovery_frontend" },
...
const MAIN_LAYER_RULES = [
  ...
  { match: /(place|maps|geo|venue|location)/i, value: "location_place_enrichment" },
  { match: /(aggregator|calendar|feed|api)/i, value: "export_feed_api" },
  { match: /(framework|infra|intake)/i, value: "source_intake" },
  { match: /(scraper|fetch|crawler)/i, value: "access_fetch" },
  { match: /(parse|extract)/i, value: "parsing_extraction" },
  { match: /(frontend|discovery|web|site)/i, value: "ui_discovery_surface" }
];

const GAP_RULES = [
  { match: /(wordpress|wp-|plugin)/i, value: "wordpress_plugin_distribution" },
  { match: /(frontend|discovery|site|web)/i, value: "frontend_and_surface_design" },
  { match: /(place|maps|geo|venue|location|gastro)/i, value: "location_and_gastro_intelligence" },
  { match: /(aggregator|calendar|feed|api)/i, value: "distribution_surfaces" },
  { match: /(framework|infra|source)/i, value: "source_systems_and_families" },
  { match: /(scraper|connector|adapter|crawler)/i, value: "connector_families" }
];

const BUILD_RULES = [
  { match: /(wordpress|wp-|plugin)/i, value: "adapt_pattern" },
  { match: /(place|maps|geo|location)/i, value: "borrow_optional" },
  { match: /(framework|infra|aggregator|calendar|feed|api)/i, value: "adapt_pattern" },
  { match: /(scraper|connector|adapter|crawler)/i, value: "adapt_pattern" }
];

const PRIORITY_RULES = [
  { match: /(aggregator|framework|infra|plugin|api|feed)/i, value: "now" },
  { match: /(place|maps|location|frontend|discovery)/i, value: "soon" },
...
export function renderRunSummary({ runId, projectKey, createdAt, items, dryRun }) {
  const lines = items.map((item) => {
    ...
    return `- ${item.repo.owner}/${item.repo.name} -> ${item.intakeDocRelativePath} (${item.action}; enrichment=${enrichment})`;
  });
