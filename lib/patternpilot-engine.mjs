...
export async function loadLandkarteEntries(rootDir, config) {
  try {
    const { rows } = await readQueue(path.join(rootDir, config.landkarteFile ?? "knowledge/repo_landkarte.csv"));
    return rows;
  } catch {
    return [];
  }
...
async function loadKnownRepoUrls(rootDir, config, project) {
  const known = new Set();
  const landkarteRaw = await safeReadText(path.join(rootDir, config.landkarteFile ?? "knowledge/repo_landkarte.csv"));
  if (landkarteRaw) {
...
export async function upsertLandkarteEntry(rootDir, config, landkarteRow, dryRun = false) {
  const landkartePath = path.join(rootDir, config.landkarteFile ?? "knowledge/repo_landkarte.csv");
