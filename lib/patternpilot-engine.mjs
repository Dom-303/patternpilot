...
export async function discoverGithubCandidates(
      ...
      plan.plans.map(async (queryPlan) => {
        try {
          const response = await searchGithubRepositories(config, queryPlan, {
            perPage: Math.max(8, Math.min((options.limit ?? 8) * 2, 20)),
            attempts: 1,
            timeoutMs: 4500
          });
  ...
  const enrichmentPoolSize = Math.max((options.limit ?? 8) * 2, 12);
  ...
  return {
    ...
    searchErrors,
    candidates: candidates.slice(0, options.limit && options.limit > 0 ? options.limit : 8)
  };
}
