...
function renderHtmlSection(title, body, tone = "default") {
  return `<section class="section-card ${tone}">
...
function renderWatchlistTopCards(review, reportView) {
  ...
  return `<div class="repo-grid">${visible.map((item) => `<article class="repo-card">
  <div class="repo-head">
...
function renderCoverageCards(coverage) {
  ...
  return `<div class="coverage-grid">${groups.map((group) => `<article class="coverage-card">
  <h3>${escapeHtml(group.title)}</h3>
  ${renderHtmlList(group.items, "none")}
</article>`).join("")}</div>`;
}
...
function renderHtmlDocument({ title, eyebrow, subtitle, lead, stats, sections }) {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .stat-card, .section-card, .coverage-card, .repo-card {
...
export function renderDiscoveryHtmlReport({
  ...
  const sections = [
    ...
    renderHtmlSection(
      "Candidate overview",
      renderDiscoveryCandidateCards(discovery.candidates, view)
    )
  ];
  ...
  if (view.showQueries) {
    sections.push(
      renderHtmlSection(
        "Discovery lenses",
        `<div class="coverage-grid">${discovery.plan.plans.map((plan) => `<article class="coverage-card">
  ...
  sections.push(
    renderHtmlSection(
      ...
      discovery.searchErrors.length > 0 ? "warn" : "default"
    )
  );
...
export function renderWatchlistReviewHtmlReport(review, reportView = "standard") {
  ...
  const sections = [
    renderHtmlSection(
      "Top recommendations",
      renderHtmlList(review.nextSteps, "No recommendations yet."),
      "accent"
    ),
    renderHtmlSection(
      "Top compared repositories",
      renderWatchlistTopCards(review, view)
    )
  ];
  ...
  if (view.showCoverage) {
    sections.push(
      renderHtmlSection(
        "Coverage",
        renderCoverageCards(review.coverage)
      )
    );
  }
  ...
  sections.push(
    renderHtmlSection(
      ...
      review.riskiestItems.length > 0 ? "warn" : "default"
    )
  );
  ...
  sections.push(
    renderHtmlSection(
      "Missing watchlist intake",
      renderHtmlList(review.missingUrls, "All current watchlist URLs already have queue coverage.")
    )
  );
