...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    problem: null,
    seedStrategy: "manual",
    patternFamilyStrategy: "off",
    slow: false,
    autoDiscover: false,
    print: false,
    reconfigure: false,
    replay: null,
    withLlmHandoff: false
  };
  ...
  while (args.length > 0) {
    ...
    if (token === "--seed-strategy") {
      // Steuert die Phase-1-Seed-Diversifikation:
      //   manual (default): derived query_seeds werden 1:1 uebernommen — identisch zum vor-Phase-1-Verhalten.
      //   auto:             Diversifier greift, wenn <3 orthogonale Seeds vorhanden sind, und supplementiert aus dem Dictionary.
      //   off:              Synonym fuer manual, defensiv.
      const raw = (args.shift() ?? "").toLowerCase();
      if (!["manual", "auto", "off"].includes(raw)) {
        console.error(`--seed-strategy erwartet auto|manual|off, bekam: ${raw}`);
        process.exit(2);
      }
      options.seedStrategy = raw === "off" ? "manual" : raw;
      continue;
    }
    if (token === "--auto-discover") {
      // Phase-4-Layer-2-Flag: bei leerer/dunner Watchlist auto-getriggertes
      // Discovery vor dem Review. Default off — Layer 1 (Watchlist-Health-
      // Diagnose) laeuft immer und macht den Empty-Watchlist-Report
      // bereits wesentlich nuetzlicher (konkrete Empfehlungen statt
      // 15 leeren Sections). Layer 2 (echter Auto-Trigger) folgt in einem
      // separaten Commit nach Real-World-Validierung.
      options.autoDiscover = true;
      continue;
    }
    if (token === "--no-auto-discover") {
      options.autoDiscover = false;
      continue;
    }
    if (token === "--slow") {
      // Phase-3-Hard-Throttle: mindestens 1000ms Pause zwischen Enrichment-
      // Calls. Opt-in fuer Nutzer ohne Premium-Token, die sonst im Secondary-
      // Rate-Limit landen. Default off.
      options.slow = true;
      continue;
    }
    if (token === "--pattern-family") {
      // Steuert die Phase-2-Pattern-Family-Klassifikation fuer problem:explore:
      //   off (default): Repos gehen unveraendert ins Stage-1-Structural-Clustering
      //                  — identisch zum vor-Phase-2-Verhalten.
      //   auto:          Klassifier laeuft ueber jedes Repo ohne pattern_family
      //                  und setzt ein Label aus dem Lexikon, wenn ein
      //                  Keyword-Match die Schwelle erreicht. Reduziert
      //                  "unknown"-Cluster im Landscape.
      const raw = (args.shift() ?? "").toLowerCase();
      if (!["off", "auto"].includes(raw)) {
        console.error(`--pattern-family erwartet off|auto, bekam: ${raw}`);
        process.exit(2);
      }
      options.patternFamilyStrategy = raw;
      continue;
    }
    if (token === "--print") {
      options.print = true;
    ...
    if (token === "--replay") {
      options.replay = args.shift() ?? null;
      continue;
    }
    if (token === "--with-llm-handoff") {
      options.withLlmHandoff = true;
      continue;
    }
