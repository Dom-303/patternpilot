...
function normalizeComparableValue(value) {
  ...
  try {
    return normalizeGithubUrl(normalizedValue).normalizedRepoUrl;
  } catch {
...
function normalizeList(values) {
  return [...new Set((Array.isArray(values) ? values : []).map(normalizeComparableValue).filter(Boolean))].sort();
}
...
function isSubset(values, candidateSuperset) {
  const superset = new Set(candidateSuperset);
  return values.every((value) => superset.has(value));
}

function describeEffectiveUrlDelta({
  latestSourceMode,
  previousSourceMode,
  effectiveUrlDelta,
  latestEffectiveUrls,
  previousEffectiveUrls
}) {
  const latestCount = latestEffectiveUrls.length;
  const previousCount = previousEffectiveUrls.length;
  const latestSubsetOfPrevious = isSubset(latestEffectiveUrls, previousEffectiveUrls);
  const previousSubsetOfLatest = isSubset(previousEffectiveUrls, latestEffectiveUrls);
  const explicitFollowUpComparison = latestSourceMode === "explicit_urls" && previousSourceMode === "explicit_urls";

  if (explicitFollowUpComparison && latestSubsetOfPrevious && latestCount < previousCount) {
    return {
      severity: "low",
      message: `Explicit URL scope narrowed from ${previousCount} repo(s) to ${latestCount}.`
    };
  }

  if (explicitFollowUpComparison && previousSubsetOfLatest && latestCount > previousCount) {
    return {
      severity: "low",
      message: `Explicit URL scope expanded from ${previousCount} repo(s) to ${latestCount}.`
    };
  }
...
function buildDriftSignals({
  ...
  const latestRunKind = latestManifest?.runPlan?.runKind ?? latestRecord?.runKind ?? null;
  const previousRunKind = previousManifest?.runPlan?.runKind ?? previousRecord?.runKind ?? null;
  ...
  if (effectiveUrlDelta.added.length > 0 || effectiveUrlDelta.removed.length > 0) {
    const effectiveUrlDrift = describeEffectiveUrlDelta({
      latestSourceMode,
      previousSourceMode,
      effectiveUrlDelta,
      latestEffectiveUrls,
    ...
    signals.push({
      ...
      severity: effectiveUrlDrift.severity,
      message: effectiveUrlDrift.message
    });
  }

  const latestRunWatchlistDelta = latestSourceMode === "watchlist"
    ? diffLists(latestEffectiveUrls, currentWatchlistUrls)
    : { added: [], removed: [] };
...
function buildResumeGuidance({
  ...
  if (signalIds.has("watchlist_delta")) {
    return {
      mode: "review_drift_before_promotion",
      nextAction: `Refresh the project queue before broader promotion changes: npm run re-evaluate -- --project ${projectKey} --stale-only`,
      recommendedCommand: `npm run re-evaluate -- --project ${projectKey} --stale-only`
    };
  }

  if (signalIds.has("effective_url_delta")) {
    if ((latestManifest?.sourceMode ?? latestRecord?.sourceMode ?? null) === "explicit_urls") {
      return {
        mode: "focused_follow_up_review",
        nextAction: `Treat the narrowed explicit-url scope as a focused follow-up review before broader promotion changes: npm run patternpilot -- review-watchlist --project ${projectKey}`,
...
export async function buildProjectRunDriftFromState(rootDir, {
  ...
  const latestEffectiveUrls = normalizeList(latestManifest?.effectiveUrls ?? []);
  const previousEffectiveUrls = normalizeList(previousManifest?.effectiveUrls ?? []);
  const effectiveUrlDelta = diffLists(previousEffectiveUrls, latestEffectiveUrls);
  ...
  const currentWatchlistDelta = latestManifest?.sourceMode === "watchlist"
    ? diffLists(latestEffectiveUrls, watchlistUrls)
    : { added: [], removed: [] };
  ...
  return {
    ...
    latestRun: latestRecord
      ? {
          ...latestRecord,
          runKind: latestManifest?.runPlan?.runKind ?? latestRecord?.runKind ?? null,
          sourceMode: latestManifest?.sourceMode ?? latestRecord.sourceMode ?? null,
          effectiveUrls: latestEffectiveUrls,
          review: latestReview
        }
      : null,
    previousRun: previousRecord
      ? {
          ...previousRecord,
          runKind: previousManifest?.runPlan?.runKind ?? previousRecord?.runKind ?? null,
          sourceMode: previousManifest?.sourceMode ?? previousRecord.sourceMode ?? null,
          effectiveUrls: previousEffectiveUrls,
