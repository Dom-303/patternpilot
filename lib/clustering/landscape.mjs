...
export function buildLandscape({
  ...
  // Stage 0: Pre-Clustering nach Query-Provenance. minClusterSize=3
  // haelt Mini-Gruppen (1-2 Repos aus einer Query-Linse) aus der
  // Landscape raus — die gehen in den Rest und werden via Stage 1/2
  // strukturell einsortiert.
  provenanceMinClusterSize = 3
}) {
  // Stage 0: Gruppiere zuerst nach Query-Provenance. Repos, die aus einer
  // Query-Phrase mit >= provenanceMinClusterSize Treffern kommen, bilden
  ...
  const { clusters: provenanceClusters, ungrouped } = clusterByProvenance(repos ?? [], {
    minClusterSize: provenanceMinClusterSize
  });
