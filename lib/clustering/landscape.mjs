...
import { buildAllClusterLabels } from "./labels.mjs";
...
export function buildLandscape({
  ...
  // Phase 7.3: Labels werden in einem einzigen Pass mit globaler IDF
  // berechnet, damit Tokens, die in mehreren Clustern auftauchen, ihren
  // Diskriminierungs-Score senken und nicht das Label dominieren.
  const allLabels = buildAllClusterLabels(flatClusters);
  for (let i = 0; i < flatClusters.length; i += 1) {
    const cluster = flatClusters[i];
    cluster.label = allLabels[i];
