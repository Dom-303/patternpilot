...
import {
  ...
  HEALTH_STATE,
} from "./review/watchlist-health.mjs";
...
import {
  getLearningForProject,
  getProjectGapAreaGuess,
  getProjectRelevanceGuess
} from "./legacy-project-fields.mjs";
...
export async function buildWatchlistReview(rootDir, config, project, binding, alignmentRules, projectProfile, options = {}) {
  ...
  for (const url of inputUrls) {
    ...
    const item = {
      ...
      gapArea: getProjectGapAreaGuess(row, ""),
      gapAreaCanonical: normalizeGapAreaCanonical(getProjectGapAreaGuess(row, ""), alignmentRules),
      ...
      projectRelevance: getProjectRelevanceGuess(row, ""),
      strengths: row.strengths || "",
      weaknesses: row.weaknesses || "",
      risks: parseCsvList(row.risks),
      learningForProject: getLearningForProject(row, ""),
  ...
  // Phase 4 Layer 1: Watchlist-Health-Diagnose. Wenn die Watchlist unter
  // dem Schwellwert liegt UND der Lauf gegen die Watchlist (nicht
  // selectedUrls) faehrt, bekommt der Report konkrete Discovery- und
  // Intake-Empfehlungen vor allen anderen Next-Steps. Bei selectedUrls
  // ist die Watchlist-Groesse irrelevant — der Nutzer hat den Korridor
  // schon explizit gewaehlt.
  const watchlistHealth = assessWatchlistHealth({
    watchlistCount: watchlistUrls.length,
    queueCount: queueRows.length,
  });
  const usingSelectedUrls = selectedUrls.length > 0;
  const watchlistHealthGapSignal = usingSelectedUrls
    ? null
    : buildWatchlistHealthGapSignal(watchlistHealth, binding.projectKey);
  const watchlistHealthNextSteps = usingSelectedUrls
    ? []
    : buildWatchlistHealthNextSteps(watchlistHealth, binding.projectKey);
  const nextStepsBase = buildWatchlistNextSteps(
    analysisProfile,
    topItems,
    coverage.uncoveredCapabilities,
    missingUrls,
    runFields.itemsDataStateSummary,
    usingSelectedUrls ? "selected_urls" : "watchlist"
  );
  const nextSteps = watchlistHealthNextSteps.length > 0
    ? [...watchlistHealthNextSteps, ...nextStepsBase]
    : nextStepsBase;
  if (watchlistHealthGapSignal) {
    runFields.runGapSignals = [watchlistHealthGapSignal, ...(runFields.runGapSignals ?? [])];
  }
