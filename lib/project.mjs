...
export async function loadProjectDiscoveryBenchmark(rootDir, project, binding) {
  const benchmarkFile = binding.discoveryBenchmarkFile ?? project.discoveryBenchmarkFile;
  if (!benchmarkFile) {
    return null;
  }

  try {
    const raw = await fs.readFile(path.join(rootDir, benchmarkFile), "utf8");
    return JSON.parse(raw);
  } catch (error) {
    if (error?.code === "ENOENT") {
      return null;
...
function normalizeSeedRepoRef(entry) {
  return String(entry?.repo ?? entry?.fullName ?? "")
    .trim()
...
function mergeSeedGroupEntries(...groups) {
  const merged = [];
  const seen = new Map();

  for (const group of groups) {
    for (const entry of Array.isArray(group) ? group : []) {
      const repoRef = normalizeSeedRepoRef(entry);
      if (!repoRef) {
        continue;
      }
      const existingIndex = seen.get(repoRef);
      if (existingIndex == null) {
        seen.set(repoRef, merged.length);
        merged.push({
          ...entry,
          repo: entry.repo ?? repoRef
        });
        continue;
      }
      ...
      merged[existingIndex] = {
        ...entry,
        ...previous,
        repo: previous.repo ?? entry.repo ?? repoRef,
        description: previous.description || entry.description || "",
        why: previous.why || entry.why || "",
...
function normalizeCohortId(entry) {
  return String(entry?.id ?? entry?.label ?? "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
...
function mergeSeedCohortEntries(...groups) {
  const merged = [];
  const seen = new Map();

  for (const group of groups) {
    for (const entry of Array.isArray(group) ? group : []) {
      const cohortId = normalizeCohortId(entry);
      if (!cohortId) {
        continue;
      }
      const existingIndex = seen.get(cohortId);
      if (existingIndex == null) {
        seen.set(cohortId, merged.length);
        merged.push({
          ...entry,
          id: entry.id ?? cohortId,
          label: entry.label ?? entry.id ?? cohortId
        });
        continue;
      }
      ...
      merged[existingIndex] = {
        ...entry,
        ...previous,
        id: previous.id ?? entry.id ?? cohortId,
        label: previous.label ?? entry.label ?? previous.id ?? entry.id ?? cohortId,
        description: previous.description || entry.description || "",
        why: previous.why || entry.why || "",
        ...
        repoRefs: uniqueStrings([
          ...(Array.isArray(previous.repoRefs) ? previous.repoRefs : []),
          ...(Array.isArray(entry.repoRefs) ? entry.repoRefs : [])
        ]),
        signals: uniqueStrings([
          ...(Array.isArray(previous.signals) ? previous.signals : []),
          ...(Array.isArray(entry.signals) ? entry.signals : [])
        ]),
        boundarySignals: uniqueStrings([
          ...(Array.isArray(previous.boundarySignals) ? previous.boundarySignals : []),
          ...(Array.isArray(entry.boundarySignals) ? entry.boundarySignals : [])
        ]),
        antiSignals: uniqueStrings([
          ...(Array.isArray(previous.antiSignals) ? previous.antiSignals : []),
          ...(Array.isArray(entry.antiSignals) ? entry.antiSignals : [])
        ])
      };
    }
  }
...
export function mergeDiscoverySeedPacks(...packs) {
  const validPacks = packs.filter((pack) => pack && typeof pack === "object");
  if (validPacks.length === 0) {
    return null;
  }

  const projectKey = validPacks.find((pack) => pack.projectKey)?.projectKey ?? null;
  const intent = validPacks
    .map((pack) => String(pack.intent ?? "").trim())
    .filter(Boolean)
    .join(" | ");

  return {
    projectKey,
    version: Math.max(...validPacks.map((pack) => Number(pack.version ?? 1) || 1)),
    intent,
    priorityRepos: mergeSeedGroupEntries(...validPacks.map((pack) => pack.priorityRepos)),
    referenceRepos: mergeSeedGroupEntries(...validPacks.map((pack) => pack.referenceRepos)),
    negativeRepos: mergeSeedGroupEntries(...validPacks.map((pack) => pack.negativeRepos)),
    priorityCohorts: mergeSeedCohortEntries(...validPacks.map((pack) => pack.priorityCohorts)),
    referenceCohorts: mergeSeedCohortEntries(...validPacks.map((pack) => pack.referenceCohorts))
  };
}

export async function loadProjectDiscoverySeeds(rootDir, project, binding) {
  const staticSeedsFile = binding.discoverySeedsFile ?? project.discoverySeedsFile;
  const staticCohortsFile = binding.discoveryCohortsFile ?? project.discoveryCohortsFile;
  const generatedSeedsFile =
    binding.generatedDiscoverySeedsFile
    ?? path.join("state", "discovery_seeds", `${binding.projectKey ?? project.label ?? "project"}.json`);

  if (!staticSeedsFile && !staticCohortsFile && !generatedSeedsFile) {
    return null;
  }

  const seedPacks = [];
  for (const seedsFile of [staticSeedsFile, staticCohortsFile, generatedSeedsFile].filter(Boolean)) {
    try {
      const raw = await fs.readFile(path.join(rootDir, seedsFile), "utf8");
      seedPacks.push(JSON.parse(raw));
    } catch (error) {
      if (error?.code === "ENOENT") {
        continue;
      }
  ...
  return mergeDiscoverySeedPacks(...seedPacks);
}
...
export async function loadProjectProfile(rootDir, project, binding, alignmentRules = null) {
  const projectRoot = path.resolve(rootDir, binding?.projectRoot ?? project.projectRoot);
