...
function buildGenericBinding({ projectKey, projectLabel, projectRoot, detected }) {
  ...
  return {
    ...
    discoveryPolicyFile: `projects/${projectKey}/DISCOVERY_POLICY.json`,
    analysisQuestions: [
...
export async function initializeProjectBinding(rootDir, config, options) {
  ...
  config.projects[projectKey] = {
    ...
    discoveryPolicyFile: `${projectDirRelative}/DISCOVERY_POLICY.json`,
  ...
  const outputs = [
    ...
    {
      path: path.join(rootDir, projectDirRelative, "DISCOVERY_POLICY.json"),
