...
export function buildDiscoveryPlan(binding, alignmentRules, projectProfile, options = {}) {
  ...
  const preferredFeedbackAnchors = sanitizeDiscoverySignals(discoveryFeedback?.preferredTerms ?? [], 4);
  const discoveryAnchors = uniqueStrings([
    ...preferredFeedbackAnchors.slice(0, 1),
    ...sanitizeDiscoverySignals(binding.discoveryHints ?? [], 6),
    ...preferredFeedbackAnchors.slice(1),
  ...
  addPlan({
    ...
    family: "architecture",
    priority: buildFamilyPriority("architecture", discoveryStrategy, discoveryFeedback),
    capabilityId: null,
  ...
  addPlan({
    ...
    family: "dependency",
    priority: buildFamilyPriority("dependency", discoveryStrategy, discoveryFeedback),
    capabilityId: null,
  ...
  const capabilityPlans = (alignmentRules?.capabilities ?? [])
    ...
    const capabilityTerms = uniqueStrings([
      ...
      family: "capability",
      priority: buildFamilyPriority("capability", discoveryStrategy, discoveryFeedback) + buildCapabilityPriority(capability, canonicalSignals),
