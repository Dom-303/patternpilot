...
import { compareDiscoveryCandidates } from "./search.mjs";
...
export async function discoverImportedCandidates(
  ...
  for (const [index, seed] of seeds.entries()) {
    ...
    const candidate = {
      ...
      queryFamilies: seed.queryFamilies ?? ["imported"],
      ...
      discoveryTrack: knownUrls.has(repo.normalizedRepoUrl) ? "baseline_anchor" : "new_discovery"
    };
  ...
  const shortlistedCandidates = visibleCandidates.slice(0, plan.discoveryProfile.limit);
