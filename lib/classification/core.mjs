...
function buildStrengths(guess, enrichment, projectLabel) {
  if (enrichment?.status !== "success") {
    return "Braucht eine Pruefung, weil keine Remote-Metadaten verfuegbar sind";
  }
  const bits = [];
  ...
  if (enrichment.repo.topics.length > 0) {
    bits.push(`Themen: ${enrichment.repo.topics.slice(0, 5).join(", ")}`);
  }
  if (enrichment.languages.length > 0) {
    bits.push(`Sprachen: ${enrichment.languages.slice(0, 3).join(", ")}`);
  }
  if (guess.priority === "now") {
    bits.push(`Wahrscheinlich bald entscheidungsrelevant fuer ${projectLabel}`);
  }
  return bits.join(" | ") || "Oeffentlich sichtbares Repo mit ausreichend Oberflaeche fuer ein Review";
}

function buildWeaknesses(guess, enrichment) {
  if (enrichment?.status !== "success") {
    return "Das Remote-Enrichment ist fehlgeschlagen, deshalb bleibt der Repo-Kontext noch oberflaechlich";
  }
  const bits = [];
  if (guess.category === "connector") {
    bits.push("Moeglicherweise enger Plattformfokus");
  }
  if (guess.category === "plugin") {
    bits.push("Stark auf Ausspielung fokussiert, aber wahrscheinlich kein Wahrheitskern");
  }
  if (deriveActivityStatus(enrichment) === "stale") {
    bits.push("Die Repo-Aktivitaet wirkt veraltet");
  }
  if (enrichment.repo.archived) {
    bits.push("Das Repository ist archiviert");
  }
  return bits.join(" | ") || "Braucht eine tiefere Repo-Lektuere, um die Systemtiefe sicher zu bestaetigen";
}
